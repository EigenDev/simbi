...
class Hydro:
    x1_spacing: StrOrNone = None
    x2_spacing: StrOrNone = None
    x3_spacing: StrOrNone = None
    ...
    def simulate(
        ...
        Parameters:
            ...
            x1_spacing (str): Option for a linear or log-spaced mesh on x1.
            x2_spacing (str): Option for a linear or log-spaced mesh on x2.
            x3_spacing (str): Option for a linear or log-spaced mesh on x3.
        ...
        if x1_spacing not in available_cellspacings:
            raise ValueError(
                f"cell spacing for x1 should be one of: {available_cellspacings}"
            )

        if x2_spacing not in available_cellspacings:
            raise ValueError(
                f"cell spacing for x2 should be one of: {available_cellspacings}"
            )

        if x3_spacing not in available_cellspacings:
            raise ValueError(
        ...
        if compute_mode == "gpu":
            if self.dimensionality == 1:
                if "GPU_BLOCK_X" not in os.environ:
                    os.environ["GPU_BLOCK_X"] = "128"
            elif self.dimensionality == 2:
                if "GPU_BLOCK_X" not in os.environ:
                    os.environ["GPU_BLOCK_X"] = "16"

                if "GPU_BLOCK_Y" not in os.environ:
                    os.environ["GPU_BLOCK_Y"] = "16"
            else:
                if "GPU_BLOCK_X" not in os.environ:
                    os.environ["GPU_BLOCK_X"] = "4"

                if "GPU_BLOCK_Y" not in os.environ:
                    os.environ["GPU_BLOCK_Y"] = "4"

                if "GPU_BLOCK_Z" not in os.environ:
                    os.environ["GPU_BLOCK_Z"] = "4"
        ...
        init_conditions = {
            ...
            "x1_spacing": x1_spacing.encode("utf-8"),
            "x2_spacing": x2_spacing.encode("utf-8"),
            "x3_spacing": x3_spacing.encode("utf-8"),
