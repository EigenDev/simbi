...
@pytest.mark.parametrize(
    "cls", [OrszagTang, NewtonianOrszagTang, IsothermalOrszagTang]
)
def test_bx_transverse_is_cell_centered_and_symmetric(cls) -> None:
    ...
    assert np.allclose(bx, bx[:, :1]), (
        "Bx must be constant along x (it depends only on y)"
    )
    ...
    assert np.allclose(col, expected, atol=1e-12), (
        "Bx not sampled at the cell center"
    )
    # the edge value would be exactly zero at jj=0; the center value is not.
    assert abs(col[0]) > 1e-6, (
        "Bx[0]==0 indicates edge (jj*dy) sampling regression"
    )
    # exact 180-degree point symmetry: Bx(y_j) == -Bx(y_{N-1-j}).
    assert np.max(np.abs(col + col[::-1])) < 1e-12, (
        "Bx is not antisymmetric about y=0.5"
    )
...
def test_by_transverse_is_cell_centered_and_symmetric(cls) -> None:
    ...
    assert np.allclose(by, by[:1, :]), (
        "By must be constant along y (it depends only on x)"
    )
    ...
    assert np.allclose(row, expected, atol=1e-12), (
        "By not sampled at the cell center"
    )
