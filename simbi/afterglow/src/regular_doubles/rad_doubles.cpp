...
namespace sogbo_rad {
    ...
    */
    ...
    {
        const auto frequency_for_unit_field =
    ...
    */
    constexpr double calc_total_synch_power(
        const double lorentz_factor,
        const double ub,
        const double beta
    )
    ...
    */
    constexpr double
    calc_nphotons_other(const double power, const double nu_c, const double dt)
    {
    ...
    */
    constexpr double calc_nelectrons_at_gamma_e(
        const double n_e,
    ...
    // field
    constexpr double
    calc_critical_lorentz(const double bfield, const double time)
    {
        return (6.0 * M_PI * constants::m_e * constants::c_light) /
               (constants::sigmaT * bfield * bfield * time);
    }
    ...
    */
    constexpr double
    calc_emissivity(const double bfield, const double n, const double p)
    {
        return (
            (9.6323 / 8.0 / M_PI) * (p - 1.0) / (3.0 * p - 1.0) *
            std::sqrt(3.0) * std::pow(constants::e_charge, 3) /
    ...
    )
    {
        ...
        for (int kk = 0; kk < nk; kk++) {
            if (!on_axis) {
                ...
                // If the data is 3D, then there is a real k-space to pull data
                // from
                kreal = (data_dim > 2) * kk;
            }
#pragma omp parallel
...
#pragma omp for nowait
                for (int ii = 0; ii < ni; ii++) {
                    ...
                    const double eps_b =
                        0.1;   // Magnetic field fraction of internal energy
                    const double eps_e =
                    ...
                    const auto nu_g =
                        calc_gyration_frequency(bfield);   // gyration frequency
                    ...
                    const double eps_m = calc_emissivity(
                        bfield,
                        n_e_proper,
                        p
                    );   // Emissivity per cell
                    const double power_prime =
                    ...
                    for (int fidx = 0; fidx < nf; fidx++) {
                        ...
                        const double power_cool = calc_powerlaw_flux(
                            power_prime,
                            p,
                            nu_source,
                            nu_c,
