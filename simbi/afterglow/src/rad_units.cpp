...
namespace sogbo_rad
{
    ...
    */
    ...
    const double gen_random_from_powerlaw(double a, double b, double p, double random_number)
    {
        const double g  = 1 - p;
    ...
    */
    ...
    const std::vector<double> vector_multiply(const std::vector<double> &a, const std::vector<double> &b) {
        ...
        std::transform(a.begin()+1, a.end(),
                b.begin()+1, v.begin(),  
                std::multiplies<double>()); 
        return v;
    };

    const std::vector<double> vector_subtract(const std::vector<double> &a, const std::vector<double> &b) {
        std::vector<double> v(a.size());
        std::transform(a.begin()+1, a.end(),
                b.begin()+1, v.begin(),  
                std::minus<double>()); 
    ...
    const std::vector<double> vector_add(const std::vector<double> &a, const std::vector<double> &b) {
        std::vector<double> v(a.size());
        std::transform(a.begin()+1, a.end(),
                b.begin()+1, v.begin(),  
                std::plus<double>()); 
    ...
    const std::vector<double> scale_vector(const std::vector<double> &a, const double scalar) {
        ...
        for (int kk = 0; kk < x3.size(); kk++) {
            ...
            for (int jj = 0; jj < x2.size(); jj++) {
                ...
                for (int ii = 0; ii < x1.size(); ii++) {
                    ...
                    // cos of the resulting beamed angle in the plane of rhat and nhat prime
                    const double mu_rhat_beam  = (mu_rhat_prime + beta) / (1.0 + beta * mu_rhat_prime); 
                    const double rot_angle     = std::acos(mu_rhat_prime) - std::acos(mu_rhat_beam);  // rotation angle from initial emission direction to beaming direction
                    const auto nhat_beamed     = scale_vector(nhat_prime, std::cos(rot_angle));
                    const auto nvec_lab        = scale_vector(nhat_beamed, x1[ii]);
                    const std::vector<double> x_mu = {t_prime.value, nvec_lab[0], nvec_lab[1], nvec_lab[2]};
