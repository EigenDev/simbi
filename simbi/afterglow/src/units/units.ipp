...
namespace units {
    ...
    class Kelvin
    {
      public:
        ...
        Kelvin(Fahrenheit<P> x)
            : value((32.0 * x.value - 32.0) * (5.0 / 9.0) + 273.15)
        {
        }
    ...
    class Fahrenheit
    {
      public:
        ...
        Fahrenheit(Kelvin<P> x)
            : value(((x.value - 273.15) * (5.0 / 9.0) + 32.0) / 32.0)
        {
        }

        Fahrenheit(Celcius<P> x) : value(x.value * (9.0 / 5.0) + 32.0) {}

        Fahrenheit(ElectronVolt<P> x)
            : value(8.4365061248733e-23 * x.value * (9.0 / 5.0) + 32.0)
        {
        }
    ...
    class Celcius
    {
      public:
        ...
        // friend std::ostream& operator<<(std::ostream& out, const Celcius<P>&
        // ell);
        P value;
    };
    ...
    class ElectronVolt
    {
      public:
        ...
        ElectronVolt(Fahrenheit<P> x)
            : value(
                  ((32 * x.value - 32.0) * 5.0 / 9.0 + 273.15) *
                  (1.0 / 1.160451812e4)
              )
        {
        }

        ElectronVolt(Celcius<P> x)
            : value((x.value - 273.15) * (1.0 / 1.160451812e4))
        {
        }
    ...
    */
    ...
    struct quantity {
            ...
            A2> uTransform) const
        {
            ...
            if ((mType == uTransform.mType) && (lType == uTransform.lType) &&
                (tType == uTransform.tType) && (qType == uTransform.qType) &&
    ...
    struct quantity<
        ...
        std::ratio<0>,
        std::ratio<0>> {
        constexpr quantity(P initVal = 0)
            : value(initVal) {}   // allow implicit conversion

        constexpr operator P() const
        {
            return value;
        }   // to/from values of type P
    ...
    constexpr auto km = quantity<
        ...
        std::ratio<0>,
        Mass_t::Gram,
        Length_t::Kilometer>(1);
    constexpr auto m = quantity<
        double,
        std::ratio<0>,
        std::ratio<1>,
        std::ratio<0>,
    ...
    constexpr auto ly = quantity<
        ...
        std::ratio<0>,
        Mass_t::Gram,
        Length_t::Lightyear>(1);
    constexpr auto pc = quantity<
        double,
        std::ratio<0>,
        std::ratio<1>,
        std::ratio<0>,
    ...
    // charge
    constexpr auto coulomb = quantity<
        double,
        std::ratio<0>,
        std::ratio<0>,
        std::ratio<0>,
    ...
    // temperature
    constexpr auto celcius = quantity<
        double,
        std::ratio<0>,
        std::ratio<0>,
        std::ratio<0>,
        ...
        Length_t::Centimeter,
        Time_t::Second,
        Charge_t::StatCoulomb,
        Temperature_t::Celcius>(1);
    constexpr auto fahrenheit = quantity<
        double,
        std::ratio<0>,
        std::ratio<0>,
        std::ratio<0>,
        ...
        Time_t::Second,
        Charge_t::StatCoulomb,
        Temperature_t::Fahrenheit>(1);

    // angle
    constexpr auto deg = quantity<
        double,
        std::ratio<0>,
        std::ratio<0>,
        std::ratio<0>,
        ...
        Time_t::Second,
        Charge_t::StatCoulomb,
        Temperature_t::Kelvin,
        Irradiance_t::ErgCM2P2,
        Angle_t::Degree>(1);
