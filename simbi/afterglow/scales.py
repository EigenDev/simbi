...
def user_scale(cls: Type[T]) -> None:
    class_name = ''.join(['-'+c.lower() if c.isupper() else c for c in cls.__name__]).lstrip('-')
    USER_SCALES[class_name] = cls
    
@user_scale
class Solar:
    length_scale = constants.R_sun.cgs
    rho_scale = ( constants.M_sun / (4. / 3. * pi * length_scale ** 3) ).cgs
    e_scale = (constants.M_sun * constants.c ** 2).cgs
    pre_scale = (e_scale / (4. / 3. * pi * length_scale**3)).cgs
...
def get_scale_model(name: str) -> T:
    try:
        ...
        raise ValueError(f'{name} is not a valid scale model. Available scale models are:\n{valid_scales}')