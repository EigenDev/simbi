...
from ..styling import axis_formatter, colorbar_formatter
...
class MultidimPlotComponent(Component):
    ...
    def _draw_immersed_bodies(self) -> None:
        ...
        for body_id, body in self.state.data.immersed_bodies.items():
            ...
            if has_capability(
                body.get("capability", 0), BodyCapability.ACCRETION
            ):
    ...
    def _project_3d_data(self, var: np.ndarray) -> np.ndarray:
        ...
        depth = self.state.config["multidim"]["slice_position"]
    ...
    def render(self) -> QuadMesh:
        ...
        if not is_cartesian:
            ...
            if self.state.config["multidim"]["bipolar"]:
                ...
                xx, yy, var = self._place_at_patch(
                    xx, yy, var, quadrant, theta_shift
                )
        ...
        if isinstance(xx, np.ndarray) and isinstance(yy, np.ndarray):
            ...
            #     shading="auto",
            # )
            # coords = [mesh[f"x{i + 1}v"] for i in range(setup["dimensions"])]
            # dV = calc_cell_volume(
            #     coords,
            #     setup["coord_system"],
            #     vertices=True,
            # )
            # mass = dV * self.bridge.get_variable("rho")
            # total_mass = mass.sum()
            # print(f"Total mass: {total_mass:.5e}")
