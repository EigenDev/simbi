...
from ..core.types import Array, Bounds, PlotData
...
def calculate_log_bins(
    data_min: float, data_max: float, num_bins: int = 50
) -> Array:
...
class HistogramPlotProps(ComponentProps):
    ...
    def validate_histtype(cls, v: str, info: ValidationInfo) -> str:
        ...
        if v not in valid_options:
            raise ValueError(
                f"Histtype must be one of {valid_options}, got {v}"
            )
        return v
...
class HistogramPlotComponent(Component):
    ...
    def render(self, data: PlotData, style: StyleConfig) -> dict[str, Any]:
        ...
        if self.props.field_index >= len(data.fields):
            return {
                ...
                "power_law": self._power_law_line,
            }
    ...
    def _render_histogram(
        self, bin_edges: Array, hist_values: Array
    ) -> BarContainer:
