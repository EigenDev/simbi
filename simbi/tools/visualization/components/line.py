...
def create_line(
    ...
    line = ax.plot(x_data, y_data, label=get_field_str(label or ""), **style)[0]
    return line
...
class LinePlotProps(ComponentProps):
    ...
    def validate_field_indices(
        ...
        for idx in v:
            if idx < 0:
                raise ValueError(
                    f"Field indices must be non-negative, got {idx}"
                )
...
class LinePlotComponent(Component):
    ...
    def render(self, data: PlotData, style: StyleConfig) -> list[Line2D]:
        """Render the line plot with data."""
    ...
    def _process_fields(
        ...
        return map_with_index(
            lambda i, field_idx: self._process_field(
                data, field_idx, i, use_legend
            ),
