...
def format_temporal_plot_axes(
    ...
    apply_temporal_labels(
        ax, data, field_index, x_label, y_label, time_units, value_units
    )

    # Set scales
    set_axis_scales(ax, log_x=False, log_y=log_y)

    # Set grid (temporal plots usually benefit from a grid)
...
def auto_scale_temporal_axes(ax: Axes, data: PlotData, field_index: int) -> None:
