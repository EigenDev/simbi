...
class Census:
    ...
    `values` is (n_samples, *bin_shape, n_values) — the stored segment axis reshaped to the
    per-axis bin counts, last axis varying fastest, matching the order the segment index was
    built in. with no bin axes `bin_shape` is empty and each sample is a single global total.
    """
    ...
    def value(self, name: str) -> Array:
        """one accumulator across every sample, shaped (n_samples, *bin_shape)."""
    ...
    def assert_fully_binned(self, sample: int | None = None) -> None:
        ...
        if worst:
            where = "" if sample is not None else f" (worst of {self.n_samples} samples)"
            raise CensusError(
...
def read_census(path: str, name: str) -> Census:
    ...
    the stored `values` is (n_samples, n_segments, n_values) with the segment axis flat; it
