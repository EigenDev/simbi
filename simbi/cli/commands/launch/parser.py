...
def setup_parser(subparsers) -> None:
    ...
    worker.add_argument("--credential", dest="credential", type=int, default=None, help="the session credential (absent: read from the rendezvous file)")
    worker.add_argument("--bind", dest="bind", default=None, help="the address this worker listens on")
    worker.add_argument("--advertise", dest="advertise", default=None, help="the host peers connect to")
