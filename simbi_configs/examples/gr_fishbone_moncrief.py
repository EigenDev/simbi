...
# seeds Kerr runs.
#
# floor-less caveats, measured: (1) the a = 0 torus is intrinsically COLD
# (p/rho ~ 1e-3 in the core; hotter placements unbind the equipotential), so its
# surface cells sit close to the physicality boundary tau + D >= sqrt(D^2+|S|^2)
# and the constant discrete stationarity residual erodes them — the corona
# pressure (the pressure-matched cut) sets that margin, but corona confinement
# strong enough for long holds also ablates the torus (~ 10% pressure ratio
# destroys the core within an orbit). the config is a SCIENCE demonstrator on
# ~100 M horizons at moderate resolution, not a precision hold gate. (2) any
# rotating equilibrium is theta-STRATIFIED (dp/dtheta balances the centrifugal
# polar component), so REFLECTING theta walls are inconsistent with it and kick
# the wall cells persistently; a precision rotating hold gate needs boundary
# ghosts PINNED to the analytic state (the driven-boundary machinery), for which
# `RotatingEquilibrium` below supplies the exact surface-free reference.
#
...
#   l             = sqrt(M r_max^3) / (r_max - 3M)     (circular orbit at the pressure max)
...
# outside sits a dilute static atmosphere (NOT an equilibrium — it accretes; the
# torus core is the gated region).
#
# usage:
...
#     with DRIVEN boundaries — every ghost band pinned to the analytic state. this is
#     the precision hold gate for the rotating balance: a theta-stratified equilibrium
#     is mathematically incompatible with mirror/copy ghosts (they impose dp/dtheta = 0
#     where the state requires the centrifugal-balancing gradient), so all four faces
#     prescribe the exact continuation instead."""
...
#         float,
#         ProblemParam(
#             1.0e-2, description="p/rho at the pressure maximum (thermal margin)"
#         ),
#     ]
