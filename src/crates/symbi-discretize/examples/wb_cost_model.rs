...
fn histogram(recon: Recon, balance: Balance) -> (BTreeMap<String, usize>, f64) {
    let (k, writes) = adiabatic_hllc_plus_flux_gv::<3>(0, recon, balance, Coords::Cartesian, &[0, 1, 2]);
...
fn main() {
    ...
    for recon in [Recon::Plm, Recon::Ppm] {
        ...
        println!("  live nodes : plain {n_plain:6}   wb {n_wb:6}   ratio {:.2}x", n_wb as f64 / n_plain as f64);
        println!("  model cost : plain {c_plain:6.0}   wb {c_wb:6.0}   ratio {:.2}x", c_wb / c_plain);
        println!("  {:<12} {:>8} {:>8} {:>8}  {:>10}", "op", "plain", "wb", "delta", "cost delta");
...
fn emit_probe() {
    ...
    for (i, line) in src.lines().enumerate() {
        if line.contains("pow(") {
            ...
            for c in ctx { println!("     {}", &c[..c.len().min(110)]); }
            break;
        }
    }
}
