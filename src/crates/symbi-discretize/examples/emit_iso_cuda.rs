...
fn main() {
    ...
    let gk = iso_ghost_fill_gv(1, 1, &[0]);
    emit_gv(&out, "iso_ghost_fill_1d", 1, gk);
    ...
    let iso_f = iso_flux_gv::<1>(0);
    emit_gv(&out, "iso_face_flux_1d", 1, iso_f);
    let adi_f = adiabatic_flux_gv::<1>(0, Recon::Plm);
    emit_gv(&out, "adiabatic_face_flux_1d", 1, adi_f);
    ...
    let iso_k = iso_c2p_gv::<1>();
    emit_gv(&out, "iso_c2p_1d", 1, iso_k);
    let adi_k = adiabatic_c2p_gv::<1>();
    emit_gv(&out, "adiabatic_c2p_1d", 1, adi_k);
    ...
    let iso_ws = iso_wave_speed_map_gv(Coords::Cartesian, &[Spacing::Uniform], &[0], 1);
    emit_gv(&out, "iso_wave_speed_map_1d", 1, iso_ws);
    ...
    let rhd_k = rhd_c2p_gv::<1>(20, EosArm::IdealGamma);
    emit_gv(&out, "rhd_c2p_1d", 1, rhd_k);
    ...
    let rhd_f = rhd_flux_gv::<1>(0, EosArm::IdealGamma);
    emit_gv(&out, "rhd_face_flux_1d", 1, rhd_f);
    ...
    let mk = godunov_mass_gv(cart.0, &cart.1, &cart.2, 1);
    emit_gv(&out, "godunov_mass_1d", 1, mk);
    // one godunov-stage kernel per regime (runtime (a0, ac) SSP coefficients serve euler/rk2/rk3).
    let iek = godunov_stage_gv(
        cart.0,
        Spacetime::Minkowski,
        &cart.1,
        &cart.2,
    ...
    emit_gv(&out, "iso_godunov_stage_1d", 1, iek);
    let aek = godunov_stage_gv(
        cart.0,
        Spacetime::Minkowski,
        &cart.1,
        &cart.2,
    ...
    emit_gv(&out, "adiabatic_godunov_stage_1d", 1, aek);
    let isk = snapshot_gv(1, false);
    emit_gv(&out, "iso_snapshot_1d", 1, isk);
    let ask = snapshot_gv(1, true);
    emit_gv(&out, "adiabatic_snapshot_1d", 1, ask);
