...
use symbi_hydro::energy::{Adiabatic, EnergyModel, IsoModel};
...
fn gr_adm_at<'t>(
    ...
    with_ks_metric!(cx, spacetime, coords, "the GR CT stack", |m| adm(&m, x))
}
...
fn gr_spatial_metric_at<'t>(
    cx: TraceCx<'t>,
    spacetime: Spacetime,
    coords: Coords,
    x: Tensor<Gv<'t>, 3>,
) -> SpatialMetric<Gv<'t>, 3> {
    use symbi_geometry::Metric;
    // spinning kerr on the cartesian chart: the rank-1 kerr-schild update with the
    // oblate-spheroidal radius; non-diagonal gamma + shift on every axis.
    fn adm<'t, M: Metric<Gv<'t>, 3>>(m: &M, x: Tensor<Gv<'t>, 3>) -> SpatialMetric<Gv<'t>, 3> {
        SpatialMetric::new(
...
fn gr_plane_pos<'t>(coords: Coords, axes: &[usize], p0: Gv<'t>, p1: Gv<'t>) -> Tensor<Gv<'t>, 3> {
...
fn ct_contact_emf_gv<'t>(face_e: [Gv<'t>; 4], cell_e: [Gv<'t>; 4], dflux: [Gv<'t>; 4]) -> Gv<'t> {
...
fn ct_curl_metric_gv<'t>(
    ...
    let p1 = (dir + 1) % 3;
    let p2 = (dir + 2) % 3;
    let pos_at = |off: [i64; 3]| {
        (0..3)
            .map(|ax| gv_axis_face_at(cx, ax, spacing[ax], off[ax]))
            .collect::<Vec<_>>()
    };
    ...
    let center: Vec<Gv> = (0..3)
        .map(|ax| {
            if ax == dir {
                gv_axis_face_at(cx, ax, spacing[ax], 0)
            } else {
                (gv_axis_face_at(cx, ax, spacing[ax], 0) + gv_axis_face_at(cx, ax, spacing[ax], 1))
                    * half
            }
        })
        .collect();
    ...
    let inv_dx_p2 = Gv::ONE
        / (gv_axis_face_at(cx, p2, spacing[p2], 1) - gv_axis_face_at(cx, p2, spacing[p2], 0));
    CtCurlMetricGv {
        h1_here,
        h1_p2,
        h2_here,
...
/// the 2.5D in-plane CT curl B-update along a single face axis `dir` from the single
/// out-of-plane corner EMF Ez (cartesian), in-place on `b` (bface[dir]). one kernel per dir
...
pub fn rmhd_ct_curl_2d_dir_gv(dir: SweepAxis) -> KernelProgram {
    ...
    trace_kernel(|cx| {
        ...
        let dt = cx.scalar("dt");
        let b_new = if dir == 0 {
            let idy = cx.scalar("idy");
            let ez_jp = ct_field_at(
                cx,
...
pub fn rmhd_resistive_emf_2d_gv() -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let bx_jm = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::A), CtWireName::Bx),
            2,
            &[0, -1],
        ); // B_x at the neighbor below in y
        let by_im = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::B), CtWireName::By),
            2,
            &[-1, 0],
...
pub fn rmhd_resistive_emf_3d_dir_gv(dir: SweepAxis) -> KernelProgram {
    ...
    trace_kernel(|cx| {
        ...
        let eta = cx.scalar("eta");
        let id_p1 = cx.scalar("id_p1");
        let id_p2 = cx.scalar("id_p2");
        let b_p1 = ct_field(
            cx,
        ...
        let b_p1_m = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::A), CtWireName::BP1),
            3,
            &back(p2),
        ); // dB_p1/dx_p2, backward
        let b_p2_m = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::B), CtWireName::BP2),
            3,
            &back(p1),
...
pub fn rmhd_ct_curl_cyl_rz_gv(dir: SweepAxis, spacing: &[Spacing]) -> KernelProgram {
    ...
    trace_kernel(|cx| {
        ...
        // registration order and keeps every registered slot, so each dir branch registers the full
        ...
        for ax in 0..2 {
            let _ = gv_axis_face_at(cx, ax, spacing[ax], 0);
            let _ = gv_axis_face_at(cx, ax, spacing[ax], 1);
        }
        let b_new = if dir == 0 {
            // dB_r/dt = +d_z E_phi : finite difference along grid axis 1 (z). no metric (h_z = 1).
            let inv_dz = Gv::ONE
                / (gv_axis_face_at(cx, 1, spacing[1], 1) - gv_axis_face_at(cx, 1, spacing[1], 0));
            let ez_zp = ct_field_at(
                cx,
                CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ez),
                2,
                &[0, 1],
            ...
            let inv_dr = Gv::ONE
                / (gv_axis_face_at(cx, 0, spacing[0], 1) - gv_axis_face_at(cx, 0, spacing[0], 0));
            let r_lo = gv_axis_face_at(cx, 0, spacing[0], 0);
            let r_hi = gv_axis_face_at(cx, 0, spacing[0], 1);
            let r_c = (r_lo + r_hi) * Gv::from_f64(0.5);
            let ez_rp = ct_field_at(
                cx,
                CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ez),
                2,
                &[1, 0],
...
pub fn rmhd_resistive_emf_cyl_rz_gv(spacing: &[Spacing]) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let br = ct_field(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::A), CtWireName::Br),
        );
        let bz = ct_field(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::B), CtWireName::Bz),
        );
        let br_zm = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::A), CtWireName::Br),
            2,
            &[0, -1],
        ); // B_r at the neighbor below in z (axis 1)
        let bz_rm = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::B), CtWireName::Bz),
            2,
            &[-1, 0],
        ); // B_z at the neighbor behind in r (axis 0)
        ...
        vec![KernelWrite::new(
            "ephi_new",
            CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ephi),
            ephi_new.node(),
        )]
    })
}
...
/// curl — written through the scale factors, so a single kernel covers every 2.5D orthogonal chart:
...
pub fn rmhd_resistive_emf_ortho_gv(coords: Coords, spacing: &[Spacing]) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let dx0 = gv_axis_face_at(cx, 0, spacing[0], 1) - gv_axis_face_at(cx, 0, spacing[0], 0);
        let dx1 = gv_axis_face_at(cx, 1, spacing[1], 1) - gv_axis_face_at(cx, 1, spacing[1], 0);
        let inv_dx0 = Gv::ONE / dx0;
        let inv_dx1 = Gv::ONE / dx1;
        let b0 = ct_field(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::A), CtWireName::B0),
        ); // bface[0], normal to in-plane axis 0
        let b1 = ct_field(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::B), CtWireName::B1),
        ); // bface[1], normal to in-plane axis 1
        let b0_jm = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::A), CtWireName::B0),
            2,
            &[0, -1],
        ); // B_0 one cell back in axis 1
        let b1_im = ct_field_at(
            cx,
            CtScratchKey::face(CtFaceCt::BFace(Transverse::B), CtWireName::B1),
            2,
            &[-1, 0],
        ); // B_1 one cell back in axis 0
        ...
        let geo = cell_geometry_gv(cx, coords, spacing, &[0usize, 1usize], 2);
        let (c0, c1) = (geo.centroid[0], geo.centroid[1]);
        let half = Gv::from_f64(0.5);
        let sf = |x0, x1| crate::gv_viscous::scale_factors_at(coords, 2, &[x0, x1]);
        ...
        vec![KernelWrite::new(
            "e_new",
            CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::E),
            e_new.node(),
        )]
    })
}
...
pub fn rmhd_ct_curl_cyl_rphi_gv(dir: SweepAxis, spacing: &[Spacing]) -> KernelProgram {
    ...
    trace_kernel(|cx| {
        ...
        let b_new = if dir == 0 {
            ...
            let r_face = gv_axis_face_at(cx, 0, spacing[0], 0);
            let inv_dphi = Gv::ONE
                / (gv_axis_face_at(cx, 1, spacing[1], 1) - gv_axis_face_at(cx, 1, spacing[1], 0));
            let ez_phip = ct_field_at(
                cx,
                CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ez),
                2,
                &[0, 1],
            ...
            // dB_phi/dt = +d_r E_z : finite difference along grid axis 0 (r). the phi-component of the
            // cyl curl is metric-free, and the discrete d-of-d still cancels (proven).
            let inv_dr = Gv::ONE
                / (gv_axis_face_at(cx, 0, spacing[0], 1) - gv_axis_face_at(cx, 0, spacing[0], 0));
            let ez_rp = ct_field_at(
                cx,
...
pub fn rmhd_ct_curl_2d_sph_gv(dir: SweepAxis, spacing: &[Spacing]) -> KernelProgram {
    ...
    trace_kernel(|cx| {
        ...
        let b_new = if dir == 0 {
            ...
            let r_f = gv_axis_face_at(cx, 0, spacing[0], 0);
            let th_lo = gv_axis_face_at(cx, 1, spacing[1], 0);
            let th_hi = gv_axis_face_at(cx, 1, spacing[1], 1);
            let th_c = (th_lo + th_hi) * half;
            let inv_dth = Gv::ONE / (th_hi - th_lo);
            let ez_thp = ct_field_at(
                cx,
                CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ez),
                2,
                &[0, 1],
...
/// discrete divergence d_i Btilde^i telescopes to zero for arbitrary per-face constant weights —
...
pub fn rmhd_ct_curl_2d_sph_gr_gv(
    ...
    trace_kernel(|cx| {
        ...
        let half = Gv::from_f64(0.5);
        let sqrtg = |p0, p1| gr_adm_at(cx, spacetime, coords, gr_plane_pos(coords, axes, p0, p1)).1;
        let b_new = if dir == 0 {
            // B_a0 lives on the low a0-face; a1_lo/a1_hi are the bounding corners of the a1 axis.
            let a0_f = gv_axis_face_at(cx, 0, spacing[0], 0);
            let a1_lo = gv_axis_face_at(cx, 1, spacing[1], 0);
            let a1_hi = gv_axis_face_at(cx, 1, spacing[1], 1);
            let a1_c = (a1_lo + a1_hi) * half;
            let w = sqrtg(a0_f, a1_c) * (a1_hi - a1_lo);
            let ez_a1p = ct_field_at(
                cx,
                CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ez),
                2,
                &[0, 1],
            ...
            let a0_lo = gv_axis_face_at(cx, 0, spacing[0], 0);
            let a0_hi = gv_axis_face_at(cx, 0, spacing[0], 1);
            let a1_f = gv_axis_face_at(cx, 1, spacing[1], 0);
            let a0_c = (a0_lo + a0_hi) * half;
            let w = sqrtg(a0_c, a1_f) * (a0_hi - a0_lo);
            let ez_a0p = ct_field_at(
                cx,
                CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::Ez),
                2,
                &[1, 0],
...
pub fn rmhd_ct_curl_3d_dir_gv(coords: Coords, spacing: &[Spacing], dir: usize) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let b = ct_field(cx, CtFaceCt::BFaceSweep);
        let dt = cx.scalar("dt");
        let ids = cartesian.then(|| (cx.scalar("id_p1"), cx.scalar("id_p2")));
        let metric = (!cartesian).then(|| ct_curl_metric_gv(cx, coords, spacing, dir));
        ...
        let curl = if let Some(m) = metric {
            ...
            let de = |key: &str, runtime: CtEdgeCt, ax: usize, w_here, w_plus, inv_dx| {
                let e_h = gv_field_at(cx, key, runtime, 3, &[0, 0, 0]);
                let e_p = gv_field_at(cx, key, runtime, 3, &off(ax));
                (w_plus * e_p - w_here * e_h) * inv_dx
            };
            let de1 = de(
                &CtWireName::EP1.render(),
                CtEdgeCt::EmfIncident(Transverse::A),
                p2,
                m.h1_here,
                m.h1_p2,
                m.inv_dx_p2,
            ...
            let de2 = de(
                &CtWireName::EP2.render(),
                CtEdgeCt::EmfIncident(Transverse::B),
                p1,
                m.h2_here,
                m.h2_p1,
                m.inv_dx_p1,
            ...
            let ddx = |key: &str, runtime: CtEdgeCt, ax: usize, inv| {
                let h = gv_field_at(cx, key, runtime, 3, &[0, 0, 0]);
                let p = gv_field_at(cx, key, runtime, 3, &off(ax));
                inv * (p - h)
            };
            let de1 = ddx(
                &CtWireName::EP1.render(),
...
pub fn imhd_bcell_from_bface_gv(ndim: usize) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let bf: Vec<Gv> = (0..ndim)
            .map(|c| cx.field(&format!("bf_{c}"), CtFaceCt::BFaceComp(PhysComp::new(c))))
            .collect();
        let writes = (0..ndim)
            .map(|c| {
                let bcc_n = (bf[c]
...
/// its two bounding faces — used for reconstruction + the c2p magnetic-energy subtraction. the
...
pub fn rmhd_edge_emf_gv(ndim: usize, g1: usize, g2: usize) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        gv_register_field(cx, "edge_bp1", CtCellCt::BCell(PlaneComp::P1));
        gv_register_field(cx, "edge_bp2", CtCellCt::BCell(PlaneComp::P2));
        gv_register_field(cx, "edge_bflux_a", CtFaceCt::BFlux(Transverse::A));
        gv_register_field(cx, "edge_bflux_b", CtFaceCt::BFlux(Transverse::B));
        gv_register_field(cx, "edge_fden_p1", CtFaceCt::FDen(Transverse::A));
        gv_register_field(cx, "edge_fden_p2", CtFaceCt::FDen(Transverse::B));
        ...
        let cell = |o: &[i32]| {
            let vp1 = gv_field_at(cx, "edge_vp1", CtCellCt::Vel(PlaneComp::P1), ndim, o);
            let vp2 = gv_field_at(cx, "edge_vp2", CtCellCt::Vel(PlaneComp::P2), ndim, o);
            let bp1 = gv_field_at(cx, "edge_bp1", CtCellCt::BCell(PlaneComp::P1), ndim, o);
            let bp2 = gv_field_at(cx, "edge_bp2", CtCellCt::BCell(PlaneComp::P2), ndim, o);
            vp2 * bp1 - vp1 * bp2
        };
        let ene = cell(&zero);
        let enw = cell(&cm(&[g1]));
...
///                measure equals the exact coordinate-form flux vtilde^n Btilde^i - vtilde^i Btilde^n,
...
pub fn rmhd_edge_emf_gr_gv(
    ...
    trace_kernel(|cx| {
        ...
        let a1_f = gv_axis_face_at(cx, 1, spacing[1], 0);
        let a0_c = |o: i64| {
            (gv_axis_face_at(cx, 0, spacing[0], o) + gv_axis_face_at(cx, 0, spacing[0], o + 1))
                * half
        };
        let a1_c = |o: i64| {
            (gv_axis_face_at(cx, 1, spacing[1], o) + gv_axis_face_at(cx, 1, spacing[1], o + 1))
                * half
        };
        ...
        let adm = |c0, c1| {
            let (alpha, sqrtg, beta) =
                gr_adm_at(cx, spacetime, coords, gr_plane_pos(coords, axes, c0, c1));
...
/// [`rmhd_bcell_from_bface_gv`]. the magnetic energy is left to tau, which carries it and is
/// conserved by the godunov flux; a `nrg += 1/2 d(gamma_ij B^i B^j)` patch would double-account
/// it and break the telescoping.
pub fn rmhd_bcell_from_bface_gr_gv(
    ...
    trace_kernel(|cx| {
        ...
        // metric-free — the cell field is a derived reconstruction / c2p quantity holding no conserved
        ...
        let writes: KernelWrites = axes
            ...
            .map(|(d, &c)| {
                let interp = (bf[d]
                    + gv_field_at(cx, &format!("bf_{d}"), &format!("bf_{d}"), ndim, &off(d)))
                    * half;
...
/// for HLL/HLLC's symmetric advection, distinct for HLLD). this struct is the only solver-specific
/// piece: HLL fills it from the fast speeds (regime-generic); HLLC/HLLD swap it for the
/// contact/alfven-aware coefficients (Eq. 38 / 44), and one master EMF kernel consumes either.
struct UctDir<'t> {
    al: Gv<'t>,
    ar: Gv<'t>,
    dl: Gv<'t>,
...
fn weighted_average<'t>(ap: Gv<'t>, vp: Gv<'t>, am: Gv<'t>, vm: Gv<'t>) -> Gv<'t> {
    let sum = ap + am;
...
fn uct_hll_coeffs<'t>(ap: Gv<'t>, am: Gv<'t>) -> UctDir<'t> {
    let sum = ap + am;
    let nonzero = sum.cmp_gt(Gv::ZERO);
    let divisor = Gv::select(nonzero, sum, Gv::ONE);
...
fn uct_hllc_coeffs<'t>(
    ...
    // Eq. 38: d^s = ((|lstar| - |lambda^s|)/2) chi^s + |lstar|/2  (the final term is |lstar|, the
    ...
    // the stable range is [0, d_HLL]: the HLL diffusion is the upper bound, holding HLLC at or below
    // HLL dissipation, and 0 is the lower bound that keeps the coefficient dissipative. bounding
    // also tames the fan degeneracy lambda^s -> lstar where chi^s blows up (an approximate
    // edge-level lstar can push d^s hugely negative -> anti-diffusion -> blow-up; a proper per-face
    // lstar stays positive there, and the guard covers the approximate one regardless).
    // floor (d >= 0): the diffusion coefficient stays dissipative. with the per-face lstar, d^s can
    // still dip slightly negative where lstar approaches lambda^s (chi^s grows); left unfloored,
    // that anti-diffusion yields an unphysically "sharp" result along the checkerboard-prone
    // direction. flooring at 0 is the correct physical guard, and the coefficient runs uncapped
    // above — HLLC's d legitimately differs from HLL's, and capping at d_HLL artificially
    // over-diffuses.
    let floor = |d| Gv::ZERO.max(d);
    UctDir {
        al: half,
        ar: half,
        dl: floor(dl),
...
pub fn rmhd_edge_emf_uct_gv(ndim: usize, g1: usize, g2: usize) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        gv_register_field(cx, "edge_bface_b", CtFaceCt::BFace(Transverse::B));
        gv_register_field(cx, "edge_wsr1", CtFaceCt::WaveR(Transverse::A));
        gv_register_field(cx, "edge_wsl1", CtFaceCt::WaveL(Transverse::A));
        gv_register_field(cx, "edge_wsr2", CtFaceCt::WaveR(Transverse::B));
        gv_register_field(cx, "edge_wsl2", CtFaceCt::WaveL(Transverse::B));
        let cm = |axes: &[usize]| -> Vec<i32> {
            let mut o = vec![0i32; ndim];
            for &ax in axes {
                o[ax] = -1;
        ...
        let vp1 = |o: &[i32]| gv_field_at(cx, "edge_vp1", CtCellCt::Vel(PlaneComp::P1), ndim, o);
        let vp2 = |o: &[i32]| gv_field_at(cx, "edge_vp2", CtCellCt::Vel(PlaneComp::P2), ndim, o);
        ...
        let max4 = |key: &str, path: CtFaceCt| {
            let v0 = gv_field_at(cx, key, path, ndim, &zero);
            let v1 = gv_field_at(cx, key, path, ndim, &cm(&[g1]));
            let v2 = gv_field_at(cx, key, path, ndim, &cm(&[g2]));
            let v3 = gv_field_at(cx, key, path, ndim, &cm(&[g1, g2]));
            v0.max(v1).max(v2).max(v3)
        };
        let neg_min4 = |key: &str, path: CtFaceCt| {
            let v0 = zero_g - gv_field_at(cx, key, path, ndim, &zero);
            let v1 = zero_g - gv_field_at(cx, key, path, ndim, &cm(&[g1]));
            let v2 = zero_g - gv_field_at(cx, key, path, ndim, &cm(&[g2]));
            let v3 = zero_g - gv_field_at(cx, key, path, ndim, &cm(&[g1, g2]));
            v0.max(v1).max(v2).max(v3)
        };
        let apx = zero_g.max(max4("edge_wsr1", CtFaceCt::WaveR(Transverse::A)));
        let amx = zero_g.max(neg_min4("edge_wsl1", CtFaceCt::WaveL(Transverse::A)));
        let apy = zero_g.max(max4("edge_wsr2", CtFaceCt::WaveR(Transverse::B)));
        let amy = zero_g.max(neg_min4("edge_wsl2", CtFaceCt::WaveL(Transverse::B)));
        // the solver-specific coefficients; HLL here, and the only piece an hllc/hlld variant replaces.
        let ux = uct_hll_coeffs(apx, amx);
        let uy = uct_hll_coeffs(apy, amy);
        ...
        let recon = |key: &str, rt: CtFaceCt, base: &[i32], axis: usize, sign: f64| {
            let off = |d: i32| -> Vec<i32> {
                let mut o = base.to_vec();
                o[axis] += d;
                o
            };
            let q0 = gv_field_at(cx, key, rt, ndim, base);
            let qm = gv_field_at(cx, key, rt, ndim, &off(-1));
            let qp = gv_field_at(cx, key, rt, ndim, &off(1));
            let slope = minmod3((q0 - qm) * theta, half * (qp - qm), (qp - q0) * theta);
            q0 + Gv::from_f64(0.5 * sign) * slope
        };
        let by_e = recon(
            "edge_bface_b",
            CtFaceCt::BFace(Transverse::B),
            &zero,
            g1,
            -1.0,
        ); // B_y[i,j],   recon -1/2 in x
        let by_w = recon(
            "edge_bface_b",
            CtFaceCt::BFace(Transverse::B),
            &cm(&[g1]),
            g1,
            1.0,
        ); // B_y[i-1,j], recon +1/2 in x
        let bx_n = recon(
            "edge_bface_a",
            CtFaceCt::BFace(Transverse::A),
            &zero,
            g2,
            -1.0,
        ); // B_x[i,j],   recon -1/2 in y
        let bx_s = recon(
            "edge_bface_a",
            CtFaceCt::BFace(Transverse::A),
            &cm(&[g2]),
            g2,
            1.0,
        ); // B_x[i,j-1], recon +1/2 in y
        let emf = uct_master_emf(&ux, &uy, vbar_x, vbar_y, by_e, by_w, bx_n, bx_s);
        vec![KernelWrite::new(
            CtWireName::Emf.render(),
            CtEdgeCt::Emf,
            emf.node(),
...
pub fn rmhd_edge_emf_uct_gr_gv(
    ...
    trace_kernel(|cx| {
        ...
        let (alpha_c, sqrtg_c, beta_c) = gr_adm_at(
            ...
            spacetime,
            coords,
            gr_plane_pos(coords, axes, a0_f, a1_f),
        );
        ...
        let emf_phys = uct_master_emf(&ux, &uy, vtilde_r, vtilde_th, by_e, by_w, bx_n, bx_s);
...
pub fn rmhd_edge_emf_uct_gr_3d_gv(
    ...
    trace_kernel(|cx| {
        ...
        let a_f = |ax: usize, o: i64| gv_axis_face_at(cx, ax, spacing[ax], o);
        let a_c = |ax: usize, o: i64| (a_f(ax, o) + a_f(ax, o + 1)) * half;
        let mut xc = [Gv::ZERO; 3];
        xc[dir] = a_c(dir, 0);
        xc[g1] = a_f(g1, 0);
        xc[g2] = a_f(g2, 0);
        let (alpha_c, sqrtg_c, beta_c) = gr_adm_at(cx, spacetime, coords, Tensor::new(xc));
        let vtilde_r = alpha_c * vbar_x - beta_c[g1];
        let vtilde_th = alpha_c * vbar_y - beta_c[g2];
        let theta = cx.scalar("theta");
        let by_e = recon_face_to_edge(
            cx,
            ndim,
            theta,
            "edge_bface_b",
...
/// riemann input); feeding it cell-centered values instead makes the EMF fan inconsistent with the
...
fn uct_master_emf<'t>(
    ...
    // but it advects the downwind state at supersonic mach -> instability (the field-loop blow-up).
    // the retained form is the single x-upwound vbar; the literal W/E-distinct Eq:UCT_HLL2 was
    // implemented and made OT noisier (the upwinding supplies smoothing the bare master form lacks).
    let adv_x = zero_g - vbar_x * (ux.al * by_w + ux.ar * by_e);
    let dif_x = ux.dr * by_e - ux.dl * by_w;
    let adv_y = vbar_y * (uy.al * bx_s + uy.ar * bx_n);
    let dif_y = zero_g - (uy.dr * bx_n - uy.dl * bx_s);
...
/// composition is linear in the faces (the wave-speed nonlinearity lives upstream, in ux/uy),
...
/// (`rmhd_edge_emf_uct_hlld_gv` / `_gr_gv`, M&DZ Eq. 39) has its own composition, and its
...
pub fn uct_master_emf_proof_kernel(swap: bool) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let vbar_x = cx.param("vbar_x");
        let vbar_y = cx.param("vbar_y");
        let by_e = cx.param("by_e");
        let by_w = cx.param("by_w");
        let bx_n = cx.param("bx_n");
        let bx_s = cx.param("bx_s");
        ...
        let emf = if swap {
            uct_master_emf(&ux, &uy, vbar_x, vbar_y, by_w, by_e, bx_n, bx_s)
        } else {
            uct_master_emf(&ux, &uy, vbar_x, vbar_y, by_e, by_w, bx_n, bx_s)
        };
        vec![KernelWrite::new(
            CtWireName::Emf.render(),
            CtEdgeCt::Emf,
...
pub(crate) fn hlld_wave_sum_terms<'t>(
    alam_l: Gv<'t>,
    aalf_l: Gv<'t>,
    aalf_r: Gv<'t>,
    alam_r: Gv<'t>,
...
pub fn hlld_wave_sum_proof_kernel(swap: bool) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let alam_r = cx.param("alam_r");
        let bt_l = cx.param("bt_l");
        let bstar_l = cx.param("bstar_l");
        let bc = cx.param("bc");
        let bstar_r = cx.param("bstar_r");
        let bt_r = cx.param("bt_r");
        let phi = if swap {
            hlld_wave_sum_terms(
...
pub fn nmhd_edge_emf_uct_hllc_gv(ndim: usize, g1: usize, g2: usize) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        gv_register_field(cx, "e_pre", CtCellCt::Pre);
        gv_register_field(cx, "e_bp1", CtCellCt::BCell(PlaneComp::P1));
        gv_register_field(cx, "e_bp2", CtCellCt::BCell(PlaneComp::P2));
        gv_register_field(cx, "e_bout", CtCellCt::BCell(PlaneComp::Out));
        gv_register_field(cx, "e_bface_a", CtFaceCt::BFace(Transverse::A));
        gv_register_field(cx, "e_bface_b", CtFaceCt::BFace(Transverse::B));
        gv_register_field(cx, "e_wsr1", CtFaceCt::WaveR(Transverse::A));
        gv_register_field(cx, "e_wsl1", CtFaceCt::WaveL(Transverse::A));
        gv_register_field(cx, "e_wsr2", CtFaceCt::WaveR(Transverse::B));
        gv_register_field(cx, "e_wsl2", CtFaceCt::WaveL(Transverse::B));
        let cm = |axes: &[usize]| -> Vec<i32> {
            let mut o = vec![0i32; ndim];
            for &ax in axes {
                o[ax] = -1;
        ...
        let half = Gv::from_f64(0.5);
        let zero_g = Gv::ZERO;
        let rho = |o: &[i32]| gv_field_at(cx, "e_rho", CtCellCt::Rho, ndim, o);
        let vp1 = |o: &[i32]| gv_field_at(cx, "e_vp1", CtCellCt::Vel(PlaneComp::P1), ndim, o);
        let vp2 = |o: &[i32]| gv_field_at(cx, "e_vp2", CtCellCt::Vel(PlaneComp::P2), ndim, o);
        let pre = |o: &[i32]| gv_field_at(cx, "e_pre", CtCellCt::Pre, ndim, o);
        let bp1 = |o: &[i32]| gv_field_at(cx, "e_bp1", CtCellCt::BCell(PlaneComp::P1), ndim, o);
        let bp2 = |o: &[i32]| gv_field_at(cx, "e_bp2", CtCellCt::BCell(PlaneComp::P2), ndim, o);
        let bout = |o: &[i32]| gv_field_at(cx, "e_bout", CtCellCt::BCell(PlaneComp::Out), ndim, o);
        let bsq = |o: &[i32]| {
            let a = bp1(o);
            let b = bp2(o);
            let c = bout(o);
        ...
        let max4 = |key: &str, path: CtFaceCt| {
            let v0 = gv_field_at(cx, key, path, ndim, &ne);
            let v1 = gv_field_at(cx, key, path, ndim, &nw);
            let v2 = gv_field_at(cx, key, path, ndim, &se);
            let v3 = gv_field_at(cx, key, path, ndim, &sw);
            v0.max(v1).max(v2).max(v3)
        };
        let neg_min4 = |key: &str, path: CtFaceCt| {
            let v0 = zero_g - gv_field_at(cx, key, path, ndim, &ne);
            let v1 = zero_g - gv_field_at(cx, key, path, ndim, &nw);
            let v2 = zero_g - gv_field_at(cx, key, path, ndim, &se);
            let v3 = zero_g - gv_field_at(cx, key, path, ndim, &sw);
            v0.max(v1).max(v2).max(v3)
        };
        let apx = zero_g.max(max4("e_wsr1", CtFaceCt::WaveR(Transverse::A)));
        let amx = zero_g.max(neg_min4("e_wsl1", CtFaceCt::WaveL(Transverse::A)));
        let apy = zero_g.max(max4("e_wsr2", CtFaceCt::WaveR(Transverse::B)));
        let amy = zero_g.max(neg_min4("e_wsl2", CtFaceCt::WaveL(Transverse::B)));
        ...
        let face_d = |l: &[i32],
            ...
            let (vl, vr) = (
                gv_field_at(cx, vn.0, vn.1, ndim, l),
                gv_field_at(cx, vn.0, vn.1, ndim, r),
            );
            let (pl, pr) = (pre(l), pre(r));
            let (bsl, bsr) = (bsq(l), bsq(r));
            let (bnl, bnr) = (
                gv_field_at(cx, bn.0, bn.1, ndim, l),
                gv_field_at(cx, bn.0, bn.1, ndim, r),
            );
            let sr = zero_g.max(
                gv_field_at(cx, wsr_k, wsr_p, ndim, l).max(gv_field_at(cx, wsr_k, wsr_p, ndim, r)),
            );
            let sl = zero_g.min(
                gv_field_at(cx, wsl_k, wsl_p, ndim, l).min(gv_field_at(cx, wsl_k, wsl_p, ndim, r)),
            );
        ...
        let vn1 = ("e_vp1", CtCellCt::Vel(PlaneComp::P1));
        let bn1 = ("e_bp1", CtCellCt::BCell(PlaneComp::P1));
        let (dln, drn) = face_d(
            &nw,
            &ne,
            vn1,
            bn1,
            "e_wsr1",
            CtFaceCt::WaveR(Transverse::A),
            "e_wsl1",
            CtFaceCt::WaveL(Transverse::A),
        );
        let (dls, drs) = face_d(
            &sw,
            &se,
        ...
        let vn2 = ("e_vp2", CtCellCt::Vel(PlaneComp::P2));
        let bn2 = ("e_bp2", CtCellCt::BCell(PlaneComp::P2));
        let (dlw, drw) = face_d(
            &sw,
            &nw,
            vn2,
            bn2,
            "e_wsr2",
            CtFaceCt::WaveR(Transverse::B),
            "e_wsl2",
            CtFaceCt::WaveL(Transverse::B),
        );
        let (dle, dre) = face_d(
            &se,
            &ne,
...
/// reconstruction (the iso energy slot is zero-sized, so the iso trace is pressure-free); the
/// per-face fan is `E::coeffs`.
fn newtonian_edge_emf_uct_hlld_impl<E: UctHlldFan>(
    ...
    trace_kernel(|cx| {
        ...
        gv_register_field(cx, "h_vp1", CtCellCt::Vel(PlaneComp::P1));
        gv_register_field(cx, "h_vp2", CtCellCt::Vel(PlaneComp::P2));
        gv_register_field(cx, "h_bp1", CtCellCt::BCell(PlaneComp::P1));
        gv_register_field(cx, "h_bp2", CtCellCt::BCell(PlaneComp::P2));
        gv_register_field(cx, "h_bout", CtCellCt::BCell(PlaneComp::Out));
        gv_register_field(cx, "h_bface_a", CtFaceCt::BFace(Transverse::A));
        gv_register_field(cx, "h_bface_b", CtFaceCt::BFace(Transverse::B));
        gv_register_field(cx, "h_wsr1", CtFaceCt::WaveR(Transverse::A));
        gv_register_field(cx, "h_wsl1", CtFaceCt::WaveL(Transverse::A));
        gv_register_field(cx, "h_wsr2", CtFaceCt::WaveR(Transverse::B));
        gv_register_field(cx, "h_wsl2", CtFaceCt::WaveL(Transverse::B));
        let half = Gv::from_f64(0.5);
        let one = Gv::ONE;
        let zero_g = Gv::ZERO;
        let [ne, nw, se, sw] = edge_corner_offsets(ndim, g1, g2);
        // the fan is fed PLM-reconstructed L/R face states (recon_cell_to_face); cell-centered values
        // make the EMF fan inconsistent with the flux at sharp reconstruction (the OT/field-loop
        // checkerboard).
        let theta = cx.scalar("theta");
        let recon_cell = |key: &str, rt: CtCellCt, base: &[i32], naxis: usize, sign: f64| {
            recon_cell_to_face(cx, ndim, theta, key, rt, base, naxis, sign)
        };
        // reconstruct the 8-component MHD prim to one side of a face; override the normal B with the
        // staggered face value (M&DZ guideline 2, identical to nmhd_reconstruct). the coefficients read
        ...
        let face_prim = |base: &[i32], naxis: usize, nidx: usize, sign: f64, bn_face| {
            let mut mag = [
                ...
                recon_cell("h_bp2", CtCellCt::BCell(PlaneComp::P2), base, naxis, sign),
                recon_cell("h_bout", CtCellCt::BCell(PlaneComp::Out), base, naxis, sign),
            ];
            mag[nidx] = bn_face;
            let rho = recon_cell("h_rho", CtCellCt::Rho, base, naxis, sign);
            let vel = Tensor::new([
                recon_cell("h_vp1", CtCellCt::Vel(PlaneComp::P1), base, naxis, sign),
                recon_cell("h_vp2", CtCellCt::Vel(PlaneComp::P2), base, naxis, sign),
                Gv::ZERO,
            ]);
            let mut slots: Vec<Gv> = vec![rho];
            slots.extend((0..3).map(|k| vel[k]));
            if E::HAS_ENERGY {
                slots.push(recon_cell("h_pre", CtCellCt::Pre, base, naxis, sign));
            }
            MhdPrimG::<Gv, 3, E>::new(PrimG::from_slots(&slots), Tensor::new(mag))
        };
        ...
        // the coefficients pass through unclamped.
        let hlld_face = |l: &[i32], r: &[i32], naxis: usize, nidx: usize, bn_face, nhat| {
        ...
        let vp1 = |o: &[i32]| gv_field_at(cx, "h_vp1", CtCellCt::Vel(PlaneComp::P1), ndim, o);
        let vp2 = |o: &[i32]| gv_field_at(cx, "h_vp2", CtCellCt::Vel(PlaneComp::P2), ndim, o);
        let vx_e = (vp1(&ne) + vp1(&se)) * half;
        let vx_w = (vp1(&nw) + vp1(&sw)) * half;
        let vy_n = (vp2(&ne) + vp2(&nw)) * half;
        let vy_s = (vp2(&se) + vp2(&sw)) * half;
        let max4 = |key: &str, path: CtFaceCt| {
            gv_field_at(cx, key, path, ndim, &ne)
                .max(gv_field_at(cx, key, path, ndim, &nw))
                .max(gv_field_at(cx, key, path, ndim, &se))
                .max(gv_field_at(cx, key, path, ndim, &sw))
        };
        let neg_min4 = |key: &str, path: CtFaceCt| {
            (zero_g - gv_field_at(cx, key, path, ndim, &ne))
                .max(zero_g - gv_field_at(cx, key, path, ndim, &nw))
                .max(zero_g - gv_field_at(cx, key, path, ndim, &se))
                .max(zero_g - gv_field_at(cx, key, path, ndim, &sw))
        };
        let apx = zero_g.max(max4("h_wsr1", CtFaceCt::WaveR(Transverse::A)));
        let amx = zero_g.max(neg_min4("h_wsl1", CtFaceCt::WaveL(Transverse::A)));
        let apy = zero_g.max(max4("h_wsr2", CtFaceCt::WaveR(Transverse::B)));
        let amy = zero_g.max(neg_min4("h_wsl2", CtFaceCt::WaveL(Transverse::B)));
        ...
        let bn_n = gv_field_at(cx, "h_bface_a", CtFaceCt::BFace(Transverse::A), ndim, &ne);
        let bn_s = gv_field_at(cx, "h_bface_a", CtFaceCt::BFace(Transverse::A), ndim, &se);
        ...
        let bn_w = gv_field_at(cx, "h_bface_b", CtFaceCt::BFace(Transverse::B), ndim, &nw);
        let bn_e = gv_field_at(cx, "h_bface_b", CtFaceCt::BFace(Transverse::B), ndim, &ne);
...
/// the NMHD UCT-HLLD edge EMF — mignone & del zanna (2020), reproduced verbatim and floor-free.
/// the composition is the solver-agnostic master form (eq:emf2D, = `uct_master_emf`,
/// byte-identical structure to UCT-HLL); per the paper, the sole solver-specific part is the per-face
...
/// is `hlld_isothermal_coeffs` (M&DZ 2020 appendix A: the isothermal fan drops the contact mode, so
/// chi~^s uses the HLL central state and a/d/nu carry over) and the prim holds the sound speed in
...
trait WaveSumChart {
    ...
    fn corner_prelude<'t>(&mut self, cx: TraceCx<'t>, half: Gv<'t>);
    ...
    fn face_vf<'t>(&mut self, cx: TraceCx<'t>, face: EmfFace, half: Gv<'t>) -> Option<Gv<'t>>;
    ...
    fn transport<'t>(&mut self, cx: TraceCx<'t>) -> Option<(Gv<'t>, Gv<'t>, Gv<'t>, Gv<'t>)>;
}
...
fn rmhd_edge_emf_uct_hlld_impl(chart: &mut impl WaveSumChart) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        gv_register_field(cx, "e_vp1", CtCellCt::Vel(PlaneComp::P1));
        gv_register_field(cx, "e_vp2", CtCellCt::Vel(PlaneComp::P2));
        gv_register_field(cx, "e_vout", CtCellCt::Vel(PlaneComp::Out));
        gv_register_field(cx, "e_pre", CtCellCt::Pre);
        gv_register_field(cx, "e_bp1", CtCellCt::BCell(PlaneComp::P1));
        gv_register_field(cx, "e_bp2", CtCellCt::BCell(PlaneComp::P2));
        gv_register_field(cx, "e_bout", CtCellCt::BCell(PlaneComp::Out));
        ...
        let prim_face =
            |base: &[i32], naxis: usize, sign: f64, n_phys: usize, bn, t_phys: usize, bt| {
                let r = |key: &str, rt: CtCellCt| {
                    recon_cell_to_face(cx, ndim, theta, key, rt, base, naxis, sign)
                };
                let rho = r("e_rho", CtCellCt::Rho);
                let pre = r("e_pre", CtCellCt::Pre);
                let mut v = [Gv::ZERO; 3];
                v[pc1] = r("e_vp1", CtCellCt::Vel(PlaneComp::P1));
                v[pc2] = r("e_vp2", CtCellCt::Vel(PlaneComp::P2));
                v[out] = r("e_vout", CtCellCt::Vel(PlaneComp::Out));
                let mut b = [Gv::ZERO; 3];
                b[pc1] = r("e_bp1", CtCellCt::BCell(PlaneComp::P1));
                b[pc2] = r("e_bp2", CtCellCt::BCell(PlaneComp::P2));
                b[out] = r("e_bout", CtCellCt::BCell(PlaneComp::Out));
                b[n_phys] = bn;
                b[t_phys] = bt;
                MhdPrim::<Gv, 3>::new(
        ...
        let bx_n_face = gv_field_at(cx, "e_bface_a", CtFaceCt::BFace(Transverse::A), ndim, &ne);
        let bx_s_face = gv_field_at(cx, "e_bface_a", CtFaceCt::BFace(Transverse::A), ndim, &se);
        let by_w_face = gv_field_at(cx, "e_bface_b", CtFaceCt::BFace(Transverse::B), ndim, &nw);
        let by_e_face = gv_field_at(cx, "e_bface_b", CtFaceCt::BFace(Transverse::B), ndim, &ne);
        ...
        let vf_xn = chart.face_vf(cx, EmfFace::XNorth, half);
        let xn_l = prim_face(&nw, g1, 1.0, pc1, bx_n_face, pc2, by_w);
        let xn_r = prim_face(&ne, g1, -1.0, pc1, bx_n_face, pc2, by_e);
        let st_xn = chart.face_states(cx, EmfFace::XNorth, &eos, &xn_l, &xn_r);
        let vf_xs = chart.face_vf(cx, EmfFace::XSouth, half);
        let xs_l = prim_face(&sw, g1, 1.0, pc1, bx_s_face, pc2, by_w);
        let xs_r = prim_face(&se, g1, -1.0, pc1, bx_s_face, pc2, by_e);
        let st_xs = chart.face_states(cx, EmfFace::XSouth, &eos, &xs_l, &xs_r);
        let phi_x = avg2(
            wave_sum(&st_xn, pc2, by_w, by_e, vf_xn, half, zero_g),
            wave_sum(&st_xs, pc2, by_w, by_e, vf_xs, half, zero_g),
        );
        ...
        let vf_yw = chart.face_vf(cx, EmfFace::YWest, half);
        let yw_l = prim_face(&sw, g2, 1.0, pc2, by_w_face, pc1, bx_s);
        let yw_r = prim_face(&nw, g2, -1.0, pc2, by_w_face, pc1, bx_n);
        let st_yw = chart.face_states(cx, EmfFace::YWest, &eos, &yw_l, &yw_r);
        let vf_ye = chart.face_vf(cx, EmfFace::YEast, half);
        let ye_l = prim_face(&se, g2, 1.0, pc2, by_e_face, pc1, bx_s);
        let ye_r = prim_face(&ne, g2, -1.0, pc2, by_e_face, pc1, bx_n);
        let st_ye = chart.face_states(cx, EmfFace::YEast, &eos, &ye_l, &ye_r);
        let phi_y = avg2(
            wave_sum(&st_yw, pc1, bx_s, bx_n, vf_yw, half, zero_g),
            wave_sum(&st_ye, pc1, bx_s, bx_n, vf_ye, half, zero_g),
        );
...
impl WaveSumChart for FlatWaveSum {
    ...
    fn corner_prelude<'t>(&mut self, _cx: TraceCx<'t>, _half: Gv<'t>) {}
    fn face_vf<'t>(&mut self, _cx: TraceCx<'t>, _face: EmfFace, _half: Gv<'t>) -> Option<Gv<'t>> {
        None
    }
    fn face_states<'t>(
        &mut self,
    ...
    fn transport<'t>(&mut self, _cx: TraceCx<'t>) -> Option<(Gv<'t>, Gv<'t>, Gv<'t>, Gv<'t>)> {
        None
    }
}
...
/// classical velocity-chi valid only in the newtonian limit, and it breaks the
/// energy-telescoping property, which is why the wave-sum form serves here.
///
...
/// ```
/// where Phi is the exact HLLD induction-flux dissipation over the true star fields (M&DZ Eq. 39):
/// ```text
...
/// `lambda^{s}` alfven (`alf`). bounded by construction — field differences times |speed|, a pure
/// polynomial in the star fields with no floor or clamp. reduces exactly to `-F_hlld_rmhd[B_t]` in
/// 1D (verified to machine precision). the staggered transverse face fields are the riemann L/R (CT
...
struct Gr2dWaveSum<'a> {
    ...
    pc2: usize,
    out: usize,
    // traced positions stashed as raw graph node ids; readers rebrand them with cx.gv.
    corner: Option<(NodeId, NodeId)>,
    face_pos: [Option<(NodeId, NodeId)>; 4],
}
...
impl WaveSumChart for Gr2dWaveSum<'_> {
    ...
    fn corner_prelude<'t>(&mut self, cx: TraceCx<'t>, _half: Gv<'t>) {
        let r_f = gv_axis_face_at(cx, 0, self.spacing[0], 0);
        let th_f = gv_axis_face_at(cx, 1, self.spacing[1], 0);
        self.corner = Some((r_f.node(), th_f.node()));
    }
    ...
    fn face_vf<'t>(&mut self, cx: TraceCx<'t>, face: EmfFace, half: Gv<'t>) -> Option<Gv<'t>> {
        ...
        let cell = |d: usize, o: i64| {
            (gv_axis_face_at(cx, d, self.spacing[d], o)
                + gv_axis_face_at(cx, d, self.spacing[d], o + 1))
                * half
        };
        let mut c = [Gv::ZERO; 2];
        c[fa] = gv_axis_face_at(cx, fa, self.spacing[fa], 0);
        c[ca] = cell(ca, co);
        ...
        self.face_pos[face.idx()] = Some((c[0].node(), c[1].node()));
    ...
    fn face_states<'t>(
        ...
        let m = gr_spatial_metric_at(
            ...
            self.spacetime,
            self.coords,
            gr_plane_pos(self.coords, self.axes, cx.gv(c0), cx.gv(c1)),
        );
    ...
    fn transport<'t>(&mut self, cx: TraceCx<'t>) -> Option<(Gv<'t>, Gv<'t>, Gv<'t>, Gv<'t>)> {
        ...
        let (alpha_c, sqrtg_c, beta_c) = gr_adm_at(
            ...
            self.spacetime,
            self.coords,
            gr_plane_pos(self.coords, self.axes, cx.gv(r_f), cx.gv(th_f)),
        );
...
struct Gr3dWaveSum<'a> {
    ...
    g2: usize,
    // traced positions stashed as raw graph node ids; readers rebrand them with cx.gv.
    corner: Option<[NodeId; 3]>,
    face_pos: [Option<[NodeId; 3]>; 4],
}

impl Gr3dWaveSum<'_> {
    fn a_f<'t>(&self, cx: TraceCx<'t>, ax: usize, o: i64) -> Gv<'t> {
        gv_axis_face_at(cx, ax, self.spacing[ax], o)
    }
    fn a_c<'t>(&self, cx: TraceCx<'t>, ax: usize, o: i64, half: Gv<'t>) -> Gv<'t> {
        (self.a_f(cx, ax, o) + self.a_f(cx, ax, o + 1)) * half
    }
}
...
impl WaveSumChart for Gr3dWaveSum<'_> {
    ...
    fn corner_prelude<'t>(&mut self, cx: TraceCx<'t>, half: Gv<'t>) {
        let mut xc = [Gv::ZERO; 3];
        xc[self.dir] = self.a_c(cx, self.dir, 0, half);
        xc[self.g1] = self.a_f(cx, self.g1, 0);
        xc[self.g2] = self.a_f(cx, self.g2, 0);
        self.corner = Some(xc.map(|v| v.node()));
    }
    ...
    fn face_vf<'t>(&mut self, cx: TraceCx<'t>, face: EmfFace, half: Gv<'t>) -> Option<Gv<'t>> {
        ...
        let mut c = [Gv::ZERO; 3];
        c[self.dir] = self.a_c(cx, self.dir, 0, half);
        c[fa] = self.a_f(cx, fa, 0);
        c[ca] = self.a_c(cx, ca, co, half);
        let pos = Tensor::new(c);
        let (alpha, _, beta) = gr_adm_at(cx, self.spacetime, self.coords, pos);
        self.face_pos[face.idx()] = Some(c.map(|v| v.node()));
    ...
    fn face_states<'t>(
        ...
        let ids = self.face_pos[face.idx()].expect("face_vf traces the face position first");
        let pos = Tensor::new(ids.map(|id| cx.gv(id)));
        let m = gr_spatial_metric_at(cx, self.spacetime, self.coords, pos);
        hlld_rmhd_states_gr_ortho(eos, l, r, face.normal(self.g1, self.g2), &m)
    }
    fn transport<'t>(&mut self, cx: TraceCx<'t>) -> Option<(Gv<'t>, Gv<'t>, Gv<'t>, Gv<'t>)> {
        let ids = self.corner.expect("corner_prelude traces the corner first");
        let corner = Tensor::new(ids.map(|id| cx.gv(id)));
        let (alpha_c, sqrtg_c, beta_c) = gr_adm_at(cx, self.spacetime, self.coords, corner);
...
/// is read at the same offsets and the edge is first-order iff at least one of the four incident
...
pub fn fofc_emf_splice_gv(ndim: usize, g1: usize, g2: usize) -> KernelProgram {
    trace_kernel(|cx| {
        ...
        let flag_gt0 =
            |o: &[i32]| gv_field_at(cx, "flag", CtCellCt::FofcFlag, ndim, o).cmp_gt(Gv::ZERO);
        let edge_fo = flag_gt0(&zero)
            | flag_gt0(&cm(&[g1]))
            | flag_gt0(&cm(&[g2]))
            | flag_gt0(&cm(&[g1, g2]));
        let e_fo = ct_field(cx, CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::EFo));
        let e_ho = ct_field(cx, CtEdgeCt::EmfHighOrder);
        let chosen = Gv::select(edge_fo, e_fo, e_ho);
        vec![KernelWrite::new(
            CtWireName::EFo.render(),
            CtScratchKey::edge(CtEdgeCt::Emf, CtWireName::EFo),
            chosen.node(),
        )]
    })
}
...
pub fn rmhd_edge_emf_gr_3d_gv(
    ...
    trace_kernel(|cx| {
        ...
        let adm = |c_g1, c_g2| {
            let (alpha, sqrtg, beta) = gr_adm_at(cx, spacetime, coords, pos(c_g1, c_g2));
...
pub fn rmhd_ct_curl_3d_gr_dir_gv(
    ...
    trace_kernel(|cx| {
        ...
        // full geom set in axis order, and the binding holds only while every slot is registered).
        for ax in 0..3 {
            let _ = gv_axis_face_at(cx, ax, spacing[ax], 0);
            let _ = gv_axis_face_at(cx, ax, spacing[ax], 1);
        }
        ...
        let sqrtg = gr_adm_at(cx, spacetime, coords, face_pos).1;
        let off = |ax: usize| -> [i32; 3] {
            let mut o = [0, 0, 0];
            o[ax] = 1;
            o
        ...
        let de = |key: &str, runtime: CtEdgeCt, ax: usize| {
            let e_h = gv_field_at(cx, key, runtime, 3, &[0, 0, 0]);
            let e_p = gv_field_at(cx, key, runtime, 3, &off(ax));
            (e_p - e_h) * inv_dx(ax)
        };
