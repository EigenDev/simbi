...
fn cfl_inv_widths_gv<'t>(
    ...
    if coords != Coords::Cartesian {
        return cell_inv_phys_widths_gv(cx, coords, spacing, axes, ndim);
    }
    (0..ndim)
        .map(|d| {
            let map_kind = cx.scalar(&format!("map_kind_{d}"));
            Gv::cond(
                map_kind.cmp_gt(Gv::from_f64(0.5)),
                || {
                    let lo = gv_axis_face_at(cx, d, spacing[d], 0);
                    let hi = gv_axis_face_at(cx, d, spacing[d], 1);
                    Gv::ONE / (hi - lo)
                },
                || cx.scalar(&format!("inv_dx_{d}")),
            )
        })
        .collect()
}
...
fn gv_cell_center<'t>(cx: TraceCx<'t>, d: usize, spacing: &[Spacing]) -> Gv<'t> {
    ...
    Gv::cond(
        ...
        || {
            Gv::cond(
                map_kind.cmp_gt(Gv::from_f64(0.5)),
                || (lo * hi).sqrt(),
                || lo + half * cx.scalar(&format!("dx_{d}")),
            )
        },
    )
}
...
fn gv_state_finite_guard<'t>(cx: TraceCx<'t>, lambda: Gv<'t>) -> Gv<'t> {
    let probe =
        cx.field("cons_den", FieldRef::cons_den()) + cx.field("cons_nrg", FieldRef::cons_nrg());
...
/// the production GRMHD admissible-boundary projection. the anchor is the
/// eulerian rebuild — p2c from the stage-input gas primitives and the
/// candidate's cell field, admissible in the candidate magnetic slice by
/// construction. the kernel emits the projection ledger's diagnostic channels
/// (`xd_theta`, `xd_d_den`, `xd_d_nrg_seg`, `xd_d_nrg_raise`) as appended
/// outputs computed from the existing nodes; the candidate arithmetic and write
/// order are untouched, so the (D, S_i, tau) writes are byte-identical to the
/// projection with the diagnostics off, and the receipts are pure evidence.
...
fn fofc_project_gr_mhd_build(
    ...
    trace(|cx| {
        ...
        // the eulerian-rebuilt anchor: the stage-input primitives still occupy the primitive
        // fields here — FOFC restored u_stage and ran c2p before constructing the first-order
        // flux, and the candidate c2p runs later — so combine that known-physical gas state with
        // the candidate B and convert it through the same GRMHD physics used by initialization.
        // this makes the anchor admissible in the candidate magnetic slice by construction, which
        // the projection ledger's near-zero anchor-energy raise records as production evidence.
        let (a_den, s_a, e_anchor) = {
...
mod log_radius_tests {
    ...
    fn eval(cx: TraceCx, out: NodeId, values: &[(&str, f64)]) -> f64 {
    ...
    fn cell_center_is_geometric_mean_on_log_axis() {
        ...
        let (_kernel, got) = trace(|cx| {
            let node = gv_cell_center(cx, 0, &[Spacing::Log]).node();
            // spacing is a runtime scalar: select the log map (map_kind_0 = 1). the
            // `Spacing::Log` builder arg is inert — the face map reads `map_kind`.
            eval(
                cx,
                node,
                &[
    ...
    fn cell_center_is_bit_identical_on_uniform_axis() {
        ...
        let (_kernel, got) = trace(|cx| {
            let node = gv_cell_center(cx, 0, &[Spacing::Uniform]).node();
            eval(
                cx,
                node,
                &[
...
#[cfg(test)]
mod projection_ledger_tests {
    use super::*;

    /// the projection ledger's diagnostic channels are appended writes computed
    /// from existing nodes: the candidate (D, S_i, tau) writes are the identical
    /// DAG nodes whether the diagnostics are on or off, so the candidate state
    /// the projection produces is byte-identical and the receipts are pure
    /// evidence. the production kernel is the diagnostics-on build.
    #[test]
    fn diagnostics_do_not_touch_the_candidate_writes() {
        let build = |diag| {
        ...
        assert_eq!(
            writes_on.len(),
            writes_off.len() + 4,
            "the diagnostics-on build appends exactly the four ledger channels"
        );
        for (i, off) in writes_off.iter().enumerate() {
            assert_eq!(
                off.destination.name(),
                writes_on[i].destination.name(),
                "candidate write {i} destination diverges"
            );
            assert_eq!(
                off.value, writes_on[i].value,
                "candidate write {i} references a different DAG node — the diagnostics \
                 perturbed the candidate arithmetic"
            );
        }
        for extra in &writes_on[writes_off.len()..] {
            assert!(
                extra.destination.name().contains("xd_"),
                "the appended writes are the diagnostic channels: {}",
