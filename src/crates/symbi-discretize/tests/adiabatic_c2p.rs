...
// `Cons::to_primitive` at S=Gv). it is ncomp-generic: ONE `adiabatic_c2p_gv::<NCOMP>`
...
fn adiabatic_c2p_is_ncomp_generic() {
    // the SAME gv builder yields NCOMP velocity components — proves it is not 1D-locked
    // nor ndim-locked; codegen instantiates it per velocity-component count (NCOMP=3 on a
    // 2D grid is the cyl r-z swirl).
