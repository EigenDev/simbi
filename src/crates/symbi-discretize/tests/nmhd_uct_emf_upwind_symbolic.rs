...
// straight off the traced DAG at graph-build time; the numerical gate reaches the
// same invariant through a supersonic field-loop advection over N steps.
//
// the master-form uct emf kernels — the solver-agnostic `eq:emf2D` composition (nmhd/imhd HLL +
// HLLD, rmhd HLL, and the GR ortho path) — all compose through the same `uct_master_emf`, and this
// proof covers exactly those. the wave-sum HLLD EMF kernels `rmhd_edge_emf_uct_hlld_gv` /
// `rmhd_edge_emf_uct_hlld_gr_gv` assemble centered advection + a dissipative Phi (M&DZ Eq. 39);
// that wave-sum assembly sits outside the master coefficient form, so their dissipation-sign pairing
// is proven separately in `rmhd_uct_hlld_wave_sum_symbolic.rs` (via `hlld_wave_sum_proof_kernel`).
...
// anti-upwind bug (a^L paired with the downwind face) slips past the div(B)
// tests (the curl preserves div(B) for any emf) and past subsonic runs
// (a^L==a^R); this proves the pairing for every kernel straight from the graph.
// =============================================================================
...
// every field read resolves to the zero-length (offset-free) stencil key: the
// symbolic param leaves stand in for LoadAt nodes.
...
// pairing violates the invariant, so the check reports a real failure when the
// pairing breaks.
#[test]
