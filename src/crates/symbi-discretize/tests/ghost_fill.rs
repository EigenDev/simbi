...
fn emit(ndim: u8) -> (String, Vec<String>) {
    ...
    let e = KernelRun::new(iso_ghost_fill_gv(nd, nd, &axes))
        .grid(vec![1usize; nd])
        .emit_cpu();
...
fn ghost_fill_1d_emits_integer_source_coord_and_vel_sign() {
    ...
    assert!(
        ...
        "map_type must be i32:\n{src}"
    );
    assert!(src.contains("arg_0: i32"), "arg must be i32:\n{src}");
    assert!(
        src.contains("vel_sign_0: S"),
    ...
    assert!(
        src.contains(": i32 = (if (map_type_0 == 0_i32)"),
        "the shared source coord must be a CSE'd i32 select:\n{src}"
    );
    assert!(
        src.contains("(_coord_0 + arg_0)"),
        "periodic source coord missing:\n{src}"
    );
    assert!(
        src.contains("(arg_0 - _coord_0)"),
        "reflect source coord missing:\n{src}"
    );
    assert!(
        src.contains("map_type_0 == 1_i32"),
        "periodic branch test missing:\n{src}"
    );
    assert!(
        !src.contains("as f64"),
        "an index must never route through f64:\n{src}"
    );
    assert!(!src.contains("as i64"), "indices are i32, no i64:\n{src}");

    // density is a straight pullback; velocity multiplies by vel_sign_0.
    assert!(
        src.contains("* vel_sign_0)") || src.contains("vel_sign_0)"),
