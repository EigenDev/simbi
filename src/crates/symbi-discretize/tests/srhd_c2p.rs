...
fn emit(ndim: u8, max_iters: usize) -> (String, Vec<(String, String)>, Vec<String>) {
    ...
    let e = KernelRun::new(built).grid(vec![1usize; ndim as usize]).emit_cpu();
...
fn srhd_c2p_1d_lowers_and_emits() {
    ...
    assert!(field_inputs.iter().any(|(k, _)| k == "cons_den"), "missing cons_den");
    assert!(field_inputs.iter().any(|(k, _)| k == "cons_mom_0"), "missing cons_mom_0");
    assert!(field_inputs.iter().any(|(k, _)| k == "cons_nrg"), "missing cons_nrg");
    assert_eq!(scalar_params, vec!["gamma"], "the SRHD c2p takes one scalar: gamma");
    assert!(src.contains("gamma: S"), "gamma must be a scalar param (precision-generic S):\n{src}");

    // docs/design/14: the Newton emits as ONE loop (body emitted once), not an 8x
    ...
    assert!(src.contains("] = ") && src.contains("buf"), "missing writes:\n{src}");
...
fn srhd_c2p_is_dimension_generic() {
    ...
    for (k, w, want) in [(&k1, &w1, 1usize), (&k2, &w2, 2), (&k3, &w3, 3)] {
        assert!(!k.graph.has_errors(), "srhd_c2p errors: {:?}", k.graph.errors());
...
fn srhd_face_flux_1d_lowers_and_emits() {
    ...
    let e = KernelRun::new(srhd_flux_gv::<1>(0)).grid([1usize]).emit_cpu();
    let src = e.source;

    assert!(e.field_inputs.iter().any(|(key, _)| key == "prim_rho"), "missing prim_rho");
    assert!(e.field_inputs.iter().any(|(key, _)| key == "prim_v0"), "missing prim_v0");
    assert!(e.field_inputs.iter().any(|(key, _)| key == "prim_pre"), "missing prim_pre");
    assert_eq!(
        e.scalar_params,
        vec!["gamma", "theta", "mesh_adot_0", "x_lo_0", "dx_0", "mesh_vtrans_0"],
        "the SRHD flux takes gamma + the theta-MC limiter + the per-axis mesh-motion/geometry scalars",
    );
    assert!(src.contains(".sqrt()"), "missing sqrt (Lorentz / wave speeds):\n{src}");
    // a face flux is a STENCIL — PLM reconstruction reads shifted neighbours.
    assert!(src.contains("] = ") && src.contains("buf"), "missing reads/writes:\n{src}");
