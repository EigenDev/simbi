...
fn bessel_j0(x: f64) -> f64 {
    ...
    if ax < 3.0 {
        ...
        let f0 = 0.79788456 - 0.00000077 * z - 0.00552740 * z * z - 0.00009512 * z.powi(3)
            + 0.00137237 * z.powi(4) - 0.00072805 * z.powi(5) + 0.00014476 * z.powi(6);
        let th = ax - 0.78539816 - 0.04166397 * z - 0.00003954 * z * z + 0.00262573 * z.powi(3)
            - 0.00054125 * z.powi(4) - 0.00029333 * z.powi(5) + 0.00013558 * z.powi(6);
        f0 * th.cos() / ax.sqrt()
    }
}
// elementary spherical Bessel functions.
fn sph_j0(x: f64) -> f64 { if x.abs() < 1e-8 { 1.0 } else { x.sin() / x } }
fn sph_j1(x: f64) -> f64 { if x.abs() < 1e-8 { 0.0 } else { x.sin() / (x * x) - x.cos() / x } }
...
fn rayleigh(
    ...
    for c in m.bface[0].domain().iter() { m.bface[0].set(c, seed0(fs, c)); }
    for c in m.bface[1].domain().iter() { m.bface[1].set(c, seed1(fs, c)); }
...
fn still_gas() -> MhdPrim<f64, 3> {
    MhdPrim { hydro: Prim { rho: 1.0, vel: Tensor::new([0.0, 0.0, 0.0]), pre: 1.0 }, mag: Tensor::new([0.0, 0.0, 0.0]) }
}
...
fn cylindrical_bessel_decay_rate() {
    ...
    assert!(rel < 0.005, "cyl Bessel decay rate off: lambda = {lambda:.5}, k^2 = {target:.5} (rel {rel:.3e})");
}
...
fn spherical_dipole_decay_rate() {
    ...
    let sim = SimStateGeneric::<NewtonianMhd, 2, 3, Spherical, IdealGas<f64>, CpuSpace, HostMemory, f64>::build(
        ...
        .cells([N, N]).bounds([1.0, 0.5], [3.0, 2.6])
        .boundaries(Boundaries::uniform(BoundaryType::Outflow)).cfl(0.3)
        .allocate().expect("spherical sim").set_initial(|_| still_gas()).seed_faces(|_, _| 0.0).build();
    let lambda = rayleigh(
        &sim,
        // B_r on r-faces at (r_face, theta_center).
        |fs, c| { let r = face0(fs, c[0]); let th = cen1(fs, c[1]); 2.0 * sph_j1(K * r) * th.cos() / r },
        // B_theta on theta-faces at (r_center, theta_face).
        |fs, c| { let r = cen0(fs, c[0]); let th = face1(fs, c[1]); -(K * sph_j0(K * r) - sph_j1(K * r) / r) * th.sin() },
        |fs, c| { let r = face0(fs, c[0]); let th = cen1(fs, c[1]); r * r * th.sin() },
        |fs, c| { let r = cen0(fs, c[0]); let th = face1(fs, c[1]); r * th.sin() },
    );
    let target = K * K;
    let rel = (lambda - target).abs() / target;
    // ~0.9% here (dipole mode + O(dx^2)); 2% bites on a wrong metric factor (O(1)).
    assert!(rel < 0.02, "spherical dipole decay rate off: lambda = {lambda:.5}, k^2 = {target:.5} (rel {rel:.3e})");
}
