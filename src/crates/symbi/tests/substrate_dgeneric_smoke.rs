...
#[test]
fn srhd_3d_runs_and_stays_symmetric() {
    type Sim = SimState<Srhd, 3, Cartesian, IdealGas<f64>, CpuSpace, HostMemory>;
    let gamma = 5.0 / 3.0;
    let n = 16usize;
    let dx = 1.0 / n as f64;
    ...
    let mut sim = Sim::build(Srhd, IdealGas { gamma }, Cartesian)
        ...
        .boundaries(Boundaries::uniform(BoundaryType::Periodic))
        .allocate()
        .expect("3D SRHD sim construction failed")
        .set_initial(|x| {
            let r2: f64 = (0..3).map(|kk| (x[kk] - 0.5).powi(2)).sum();
    ...
    let sub = SrhdSubstrateKernelSet::<HostMemory, f64, 3>::new(gamma, 0.4, &sim.geom.allocated);
    evolve(&mut sim, &sub, 0.03).expect("3D SRHD evolution failed");
    ...
    assert!(max_asym < 1e-12, "3D SRHD x<->y / x<->z symmetry broken: max asymmetry {max_asym:e}");
    assert!(max_speed > 0.005, "3D SRHD pulse did not develop flow (max speed {max_speed})");
    println!("SRHD 3D: {} steps to t={:.3}, max asym {:e}, max speed {:.4}", sim.iteration, sim.time, max_asym, max_speed);
}

#[test]
fn srhd_2d_runs_and_stays_xy_symmetric() {
    // closes the SRHD multi-D CFL gap: the D-generic wave-speed map folds the per-axis
    // relativistic Davis speed over both axes, so SRHD runs at 2D. same x<->y symmetry
    ...
    type Sim = SimState<Srhd, 2, Cartesian, IdealGas<f64>, CpuSpace, HostMemory>;
    let gamma = 5.0 / 3.0;
    let n = 32usize;
    let dx = 1.0 / n as f64;
    ...
    let mut sim = Sim::build(Srhd, IdealGas { gamma }, Cartesian)
        ...
        .boundaries(Boundaries::uniform(BoundaryType::Periodic))
        .allocate()
        .expect("2D SRHD sim construction failed")
        .set_initial(|x| {
            let r2 = (x[0] - 0.5).powi(2) + (x[1] - 0.5).powi(2);
    ...
    let sub = SrhdSubstrateKernelSet::<HostMemory, f64, 2>::new(gamma, 0.4, &sim.geom.allocated);
    evolve(&mut sim, &sub, 0.04).expect("2D SRHD evolution failed");
    ...
    println!(
        "SRHD 2D: {} steps to t={:.3}, max asym {:e}, max speed {:.4}",
