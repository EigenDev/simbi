...
fn assert_cons_bit_identical<const D: usize>(
    ...
    for c in interior.iter() {
        ...
        assert_eq!(
            ...
            "{label} differs at {c:?}: elided={va:?} reference={vb:?} (delta={:?})", va - vb,
        );
    }
}
...
fn stage0_un_alias_matches_the_snapshot_stage_copy() {
    ...
    let build = || -> Sim {
        ...
        sim.seed_cells(|p| {
            ...
            Prim { rho, vel: Tensor::new([0.15, -0.08]), pre: 1.0 }
        });
        sim
    };
    ...
    sim_ref.workspace.elide_stage_snapshot.store(false, Ordering::Relaxed);
    let sub_ref = sim_ref.substrate()
    ...
    assert!(
        sim_elide.workspace.elide_stage_snapshot.load(Ordering::Relaxed),
        "the elision must be the production default, else this oracle compares two reference runs",
    );
    let sub_elide = sim_elide.substrate()
        .with_runtime_source(build_user_source(&cfg, &NEWTONIAN_SPEC).unwrap(), cfg.params.clone());
    ...
    assert!(
        !sim_ref.workspace.elide_stage_snapshot.load(Ordering::Relaxed),
        "reference run must have the elision disabled",
    );

    let interior = &sim_elide.geom.interior;
    assert_cons_bit_identical(interior, &sim_elide.fields.cons.den, &sim_ref.fields.cons.den, "cons.den");
    for k in 0..2 {
        assert_cons_bit_identical(interior, &sim_elide.fields.cons.mom[k], &sim_ref.fields.cons.mom[k], "cons.mom");
    }
    let (ne, nr) = (
    ...
    let moved = interior.iter().any(|c| {
        (*sim_elide.fields.cons.mom[0].view().at(c) - 0.15 * *sim_elide.fields.cons.den.view().at(c)).abs() > 1e-9
    });
    assert!(moved, "state never evolved — the comparison would be vacuous");
}
