...
// (CpuSpace / HostMemory) — the backing of the `SimDefault*` aliases. a single source file
// compiles for either backend through this alias, with the `#[cfg(feature="gpu")]` Space/Mem
// selection confined here.
...
/// — a single binary that runs on whichever backend it's built for, with the `#[cfg(feature="cuda")]`
