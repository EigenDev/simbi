...
// the CYLINDRICAL (R, phi) viscous gate: the compiled kernel is bit-identical to
// the f64 chain built from `viscous_mom_update_cyl_2d`, with the R centroid the
// volume-weighted (2/3)(rh^3-rl^3)/(rh^2-rl^2) the cell geometry produces. the
// velocity is (v_R, v_phi) physical; a Keplerian-plus-perturbation field makes the
// metric strain terms (u/R, -w/R) and the R^2 flux all bite.
#[test]
fn compiled_viscous_iso_cyl_matches_the_f64_chain_bitwise() {
    use symbi_hydro::viscous::viscous_mom_update_cyl_2d;
    let (kernel, ir) = kernel_by_name::<f64>("viscous_iso_cyl_2d").expect("cyl viscous kernel");
    ...
    // the f64 chain: the volume-weighted R centroid (gv_powi = repeated mul), then
    // the cylindrical operator over the same interior.
    ...
    for jj in 1..N - 1 {
        for ii in 1..N - 1 {
            ...
            let dmom =
                viscous_mom_update_cyl_2d(&vst, &rst, &[[NU; 3]; 3], r_c(ii), DR, DPHI, DT);
            let c = ii + jj * N;
            assert_eq!(m0[c].to_bits(), (m0_in[c] + dmom[0]).to_bits(), "mom0 ({ii},{jj})");
            assert_eq!(m1[c].to_bits(), (m1_in[c] + dmom[1]).to_bits(), "mom1 ({ii},{jj})");
    ...
    assert!(checked > 20, "the cylindrical viscous operator never produced a nonzero torque");
}

// the CYLINDRICAL alpha gate: nu(R) = alpha cs^2 / Omega_k(R) about the central
// mass on the axis (R is the orbital radius), evaluated at the operator's uniform
// neighbour radii r_c + (i-1) dr. bit-identical to the f64 chain.
#[test]
fn compiled_viscous_iso_alpha_cyl_matches_the_f64_chain_bitwise() {
    use symbi_hydro::viscous::viscous_mom_update_cyl_2d;
    let (kernel, ir) =
        kernel_by_name::<f64>("viscous_iso_alpha_cyl_2d").expect("cyl alpha viscous kernel");
    ...
    // nu at stencil offset di: R = r_c(output) + (di-1) dr, matching the kernel.
    let nu_at = |rc: f64, di: usize| {
        let r = (rc + (di as f64 - 1.0) * DR).max(1e-30);
        let omega_k = (GM / (r * r * r)).sqrt().max(1e-30);
        ALPHA * CS * CS / omega_k
    };
    let mut checked = 0usize;
        ...
        for ii in 1..N - 1 {
            ...
            for dj in 0..3 {
                for di in 0..3 {
                    ...
                    vst[dj][di] = Tensor::new([v0[c], v1[c]]);
                    rst[dj][di] = rho[c];
                    nst[dj][di] = nu_at(rc, di);
                }
            }
            let dmom = viscous_mom_update_cyl_2d(&vst, &rst, &nst, rc, DR, DPHI, DT);
            let c = ii + jj * N;
            assert_eq!(m0[c].to_bits(), (m0_in[c] + dmom[0]).to_bits(), "mom0 ({ii},{jj})");
            assert_eq!(m1[c].to_bits(), (m1_in[c] + dmom[1]).to_bits(), "mom1 ({ii},{jj})");
            if (m1[c] - m1_in[c]).abs() > 1e-14 {
