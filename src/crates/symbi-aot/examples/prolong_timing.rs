...
fn main() {
    ...
    let coarse: Vec<Vec<f64>> = (0..10)
        .map(|k| {
            (0..ncoarse)
                .map(|ii| 1.0 + 0.3 * (((ii * 7 + k * 13) % 97) as f64) / 97.0)
                .collect()
        })
        .collect();
    ...
    let run_pair = |fine: &mut [Vec<f64>]| {
        let inputs: Vec<CpuField> = (0..10)
            .map(|k| CpuField::from_layout(&coarse[k], &LO, &c_ext))
            .collect();
        let mut outs: Vec<CpuFieldMut> = fine
            .iter_mut()
            .map(|o| CpuFieldMut::from_layout(o, &LO, &f_ext))
            .collect();
    ...
    let run_split = |lerped: &mut [Vec<f64>], fine: &mut [Vec<f64>]| {
        {
            let inputs: Vec<CpuField> = (0..10)
                .map(|k| CpuField::from_layout(&coarse[k], &LO, &c_ext))
                .collect();
            let mut louts: Vec<CpuFieldMut> = lerped
                .iter_mut()
                .map(|o| CpuFieldMut::from_layout(o, &LO, &c_ext))
                .collect();
            lerp(&inputs, &mut louts, &c_ext, &LO, &[], &[alpha]);
        }
        let inputs: Vec<CpuField> = lerped
            .iter()
            .map(|l| CpuField::from_layout(l, &LO, &c_ext))
            .collect();
        let mut outs: Vec<CpuFieldMut> = fine
            .iter_mut()
            .map(|o| CpuFieldMut::from_layout(o, &LO, &f_ext))
    ...
    let f_hi = [
        LO[0] as isize + SLAB[0] as isize,
        LO[1] as isize + SLAB[1] as isize,
        LO[2] as isize + SLAB[2] as isize,
    ];
    ...
    let (sw0, _) =
        kernel_by_name::<f64>(&format!("refine_prolong_sw0_{order}_5c_3d_serial")).expect("sw0");
    let (sw1, _) =
        kernel_by_name::<f64>(&format!("refine_prolong_sw1_{order}_5c_3d_serial")).expect("sw1");
    let (sw2, _) =
        kernel_by_name::<f64>(&format!("refine_prolong_sw2_{order}_5c_3d_serial")).expect("sw2");
    let run_sweep = |lerped: &mut [Vec<f64>],
                     mid_a: &mut [Vec<f64>],
                     mid_b: &mut [Vec<f64>],
                     fine: &mut [Vec<f64>]| {
        {
        ...
        }
        {
            let inputs: Vec<CpuField> = lerped
                ...
                .map(|l| CpuField::from_layout(l, &LO, &c_ext))
                .collect();
            let mut outs: Vec<CpuFieldMut> = mid_a
                .iter_mut()
                .map(|o| CpuFieldMut::from_layout(o, &a_lo, &a_ext))
                .collect();
            sw0(&inputs, &mut outs, &a_ext, &a_lo, &[], &[]);
        }
        {
            let inputs: Vec<CpuField> = mid_a
                .iter()
                .map(|l| CpuField::from_layout(l, &a_lo, &a_ext))
                .collect();
            let mut outs: Vec<CpuFieldMut> = mid_b
                .iter_mut()
                .map(|o| CpuFieldMut::from_layout(o, &b_lo, &b_ext))
                .collect();
            sw1(&inputs, &mut outs, &b_ext, &b_lo, &[], &[]);
        }
        let inputs: Vec<CpuField> = mid_b
            .iter()
            .map(|l| CpuField::from_layout(l, &b_lo, &b_ext))
            .collect();
        let mut outs: Vec<CpuFieldMut> = fine
            .iter_mut()
            .map(|o| CpuFieldMut::from_layout(o, &LO, &f_ext))
    ...
    println!(
        "{order} on {SLAB:?} slab ({} fine zones, {reps} reps, serial):",
        nfine
    );
    println!("  time-pair kernel:       {pair_ns:8.1} ns/zone");
    println!(
        "  lerp + single-snapshot: {split_ns:8.1} ns/zone  ({:.2}x)",
        pair_ns / split_ns
    );
    println!(
        "  lerp + axis sweeps:     {sweep_ns:8.1} ns/zone  ({:.2}x)",
        pair_ns / sweep_ns
    );
}
