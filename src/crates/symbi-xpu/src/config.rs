...
pub fn extent_aware_block(ndim: usize, extent: &[u32]) -> [u32; 3] {
    ...
    let g = |a: usize| -> u32 {
        if a < ndim { extent.get(a).copied().unwrap_or(1).max(1) } else { 1 }
    };
    ...
    let grow_axis = |b: &mut [u32; 3], axis: usize, g: u32, warp: bool| {
        let others = b.iter().enumerate().filter(|&(i, _)| i != axis).map(|(_, &x)| x).product::<u32>();
        let room = (target / others.max(1)).max(1);
        let mut v = g.min(room);
        if warp && v >= WARP { v -= v % WARP; }
        b[axis] = v.max(b[axis]);
    };
...
pub fn block_dims(ndim: usize) -> [u32; 3] {
    assert!((1..=3).contains(&ndim), "block_dims: ndim {ndim} not in 1..=3");
...
fn env_block(ndim: usize) -> Option<[u32; 3]> {
    ...
    assert!((1..=3).contains(&ndim), "env_block: ndim {ndim} not in 1..=3");
    dims[ndim - 1]
}
...
fn parse_block(var: &str, want: usize) -> Option<[u32; 3]> {
    ...
    if parts.len() != want {
        eprintln!("symbi: {var}=\"{raw}\" needs {want} comma-separated u32s; ignoring (using extent-aware default)");
        return None;
    }
    let mut out = [1u32; 3];
    for (i, p) in parts.iter().enumerate() {
        match p.parse::<u32>() {
            Ok(v) if v >= 1 => out[i] = v,
            _ => {
                eprintln!("symbi: {var}=\"{raw}\" component {i} is not a positive u32; ignoring (using extent-aware default)");
                return None;
            }
        }
    }
...
mod tests {
    ...
    fn check(ndim: usize, ext: &[u32]) -> [u32; 3] {
        ...
        assert!(total <= 256, "block {b:?} over {ext:?}: {total} > 256 threads");
        for a in 0..3 {
            let g = if a < ndim { ext[a].max(1) } else { 1 };
            // CORE INVARIANT: no block dim exceeds its extent => no lane launched past a thin axis.
            assert!(b[a] >= 1 && b[a] <= g, "dim {a} of {b:?} exceeds extent {g}");
            if a >= ndim { assert_eq!(b[a], 1, "axis {a} >= ndim must be 1"); }
        }
        b
    }
