...
pub fn extent_aware_block(ndim: usize, extent: &[u32]) -> [u32; 3] {
    ...
    // a thin axis leaves the budget unspent — redistribute it to the axes with room,
    // contiguous-axis-first for coalescing (x), then y, then z. when x is the thin
    // one (e.g., a 2-wide ghost layer) the budget flows to y/z, keeping the block at
    // least a half-warp. each grow is clamped to its extent + the running budget; x
    // is warp-aligned when it reaches a warp. a fat domain has no unspent budget, so
    // it keeps its validated base shape (no regression).
...
mod tests {
    ...
    // warp multiple (or the whole axis when the axis is sub-warp), and every lane launched
    // within a thin axis (block dim <= extent).
    fn check(ndim: usize, ext: &[u32]) -> [u32; 3] {
        ...
        for a in 0..3 {
            ...
            // core invariant: every block dim stays within its extent, so every lane lands
            // within a thin axis.
            assert!(
                b[a] >= 1 && b[a] <= g,
    ...
    fn extent_aware_shapes() {
        ...
        // native 2D / 1D base shapes stay as before (Iso/Newtonian preserved).
        ...
        // axis 0 thin (a 2-wide ghost layer): x has no room to grow, so the budget flows to y and
