...
impl std::fmt::Display for SpliceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SpliceError::OutputOutOfRange => {
                write!(f, "splice: source_output is not a valid NodeId for source",)
            }
        }
    }
}
...
pub fn splice_graph(
    ...
    for (src_id, node, src_ty) in source.iter() {
        ...
        let new_id = match &node.op {
            Op::Param(sym) => {
                let target_id = param_subst
                    .get(sym)
                    .copied()
                    .ok_or_else(|| SpliceError::UnmappedParam { name: sym.clone() })?;
...
mod tests {
    use super::*;
    use crate::{ConstValue, DimExpr, ElementTy, ElementWiseOp, Symbol, TensorTy};
    ...
    fn splice_inlines_into_target_and_resolves_param() {
        ...
        match &result_node.op {
            Op::ElementWise(ElementWiseOp::Mul, inputs) => {
                ...
                assert_eq!(
                    inputs[0], sum,
                    "mul lhs should resolve to substituted NodeId"
                );
    ...
    fn splice_multi_output_shares_intermediate_nodes() {
        ...
        let (a_inv, b_inv) = match (&target.node(outs[0]).op, &target.node(outs[1]).op) {
            (Op::ElementWise(ElementWiseOp::Mul, ai), Op::ElementWise(ElementWiseOp::Mul, bi)) => {
                (ai[0], bi[0])
            }
