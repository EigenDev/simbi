...
mod tests {
    ...
    // past serialization), a float const (ConstValue bit-pattern serde), a scalar
    // param (param_elem), and a transcendental MethodCall (method: String serde).
    #[test]
    fn prepared_ir_round_trips_for_both_backends() {
        ...
        let field_inputs = [("u".to_string(), FieldBind::from_path("u"))];
        let scalar_params = ["dt".to_string()];
        ...
        let prepared = prepare(&g, &inputs);
        let json = serde_json::to_string(&prepared).expect("serialize Prepared");
        let restored: Prepared = serde_json::from_str(&json).expect("deserialize Prepared");
