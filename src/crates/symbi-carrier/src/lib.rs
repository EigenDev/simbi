...
// symbi-carrier
//
// the foundational carrier algebra — the mathematical contract every
// interpretation implements. physics is written over `S: Scalar` and depends
// on this crate alone; each carrier is a total homomorphism from the free
// algebra into a concrete one: `f64`/`f32` evaluate (defined here), `Dual<S>`
// carries derivatives (dual.rs), and the tracing interpretation lives with
// the compiler in `symbi-ir`.
//
// invariant: this crate is free of `panic!`/`unwrap`/`expect`. category
...
// boundaries (config parse, NVRTC compile, HDF5 write), outside this crate.
//
// usage:
//  fn pressure<S: Scalar>(rho: S, eint: S, gamma: S) -> S { (gamma - S::ONE) * rho * eint }
//  let d = f(Dual::variable(x0)).tangent;
// =============================================================================
...
//   `Scalar for Gv`          — crates/symbi-ir/src/gv.rs. the tracing
//                              interpretation, supplied by the compiler.
//   `Scalar for Dual<C>`     — dual.rs (this crate). forward-mode
//                              derivative carrier.
//
