// =============================================================================
// penalize_shaped_jit.rs
//
// the make-or-break gate for the runtime-JIT arbitrary-shape rigid wall: the
// setup-built shaped penalization kernel must cranelift-compile on host. its
// surface normal is the SDF gradient (Dual-derived CSG min/max branches), so
// this pins that the shaped kernel's op set lies inside the JIT subset — else
// the moving arbitrary-shape body would have no host execution path.
// =============================================================================
use symbi_discretize::coords::Coords;
use symbi_discretize::{
    penalize_porous_gv_shaped, penalize_porous_gv_spinning, penalize_porous_iso_gv_shaped,
    penalize_porous_iso_gv_spinning,
};
use symbi_ib::sdf::SdfExpr;

#[test]
fn shaped_porous_penalize_jit_compiles_on_host() {
    // a box unioned with an offset sphere — a genuine CSG with min/max kinks and a
    // Dual-autodiff normal, in the body-local frame.
    let shape = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.5, 0.3, 0.2])
        .union(SdfExpr::sphere([0.6, 0.0, 0.0], 0.25));
    let program = penalize_porous_gv_shaped(Coords::Cartesian, 3, 3, &shape, false);
    let compiled = symbi_jit::compile_gv_kernel(&program, 3);
    assert!(
        compiled.is_ok(),
        "the shaped penalize kernel is outside the cranelift JIT subset: {:?}",
        compiled.err(),
    );
}

#[test]
fn shaped_porous_penalize_jit_compiles_2d() {
    // a 2d run (r-z / x-y): the kernel is 3D internally but the active dims are 2.
    let shape = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.4, 0.6, 1.0]);
    let program = penalize_porous_gv_shaped(Coords::Cartesian, 2, 2, &shape, false);
    assert!(symbi_jit::compile_gv_kernel(&program, 2).is_ok());
}

#[test]
fn shaped_porous_penalize_jit_compiles_2p5d() {
    // dof = 3 on a 2d grid (2.5d mhd): the out-of-plane momentum channel rides the same
    // shaped kernel and must stay inside the JIT subset.
    let shape = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.4, 0.6, 1.0]);
    let program = penalize_porous_gv_shaped(Coords::Cartesian, 2, 3, &shape, false);
    assert!(symbi_jit::compile_gv_kernel(&program, 2).is_ok());
}

#[test]
fn capped_cylinder_penalize_jit_compiles_on_host() {
    let shape = SdfExpr::<f64, 3>::capped_cylinder([0.0, 0.0, 0.0], 0.4, 0.8);
    let program = penalize_porous_gv_shaped(Coords::Cartesian, 3, 3, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 3).is_ok(),
        "the capped-cylinder penalize kernel is outside the JIT subset",
    );
}

#[test]
fn shaped_rotated_penalize_jit_compiles() {
    // a rotated box (static tilt): the Rotated node is affine (multiply/add), so it must lie in
    // the JIT subset — the same runtime kernel bakes the orientation matrix as constants.
    let tilt = [
        [0.7071, -0.7071, 0.0],
        [0.7071, 0.7071, 0.0],
        [0.0, 0.0, 1.0],
    ];
    let shape = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.5, 0.2, 0.3]).rotated(tilt);
    let program = penalize_porous_gv_shaped(Coords::Cartesian, 2, 2, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 2).is_ok(),
        "the rotated shaped penalize kernel is outside the JIT subset",
    );
}

#[test]
fn shaped_penalize_jit_compiles_curvilinear() {
    // the mask distance is physical: on a spherical / cylindrical grid the kernel maps the
    // coordinate centroid to Cartesian first. that path (centroid_to_cartesian +
    // vector_from_cartesian) must lie in the JIT subset for a shaped body.
    let shape = SdfExpr::<f64, 3>::cuboid([1.5, 0.0, 0.0], [0.3, 0.3, 0.3]);
    let program = penalize_porous_gv_shaped(Coords::Spherical, 2, 2, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 2).is_ok(),
        "spherical shaped kernel not JIT-able"
    );
    let program = penalize_porous_gv_shaped(Coords::Cylindrical, 2, 2, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 2).is_ok(),
        "cylindrical shaped kernel not JIT-able"
    );
    let program = penalize_porous_iso_gv_shaped(Coords::Cylindrical, 3, 3, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 3).is_ok(),
        "cylindrical iso shaped kernel not JIT-able"
    );
}

#[test]
fn spinning_penalize_jit_compiles() {
    // the spinning wall: the mask is rotated by R(body_0_angle) built from Gv cos/sin (runtime
    // angle), and the surface velocity carries omega x r. both the adiabatic and iso kernels must
    // JIT — cos/sin are in the cranelift subset.
    let shape = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.5, 0.2, 0.3]);
    let program = penalize_porous_gv_spinning(Coords::Cartesian, 2, 2, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 2).is_ok(),
        "adiabatic spinning kernel not JIT-able"
    );
    let program = penalize_porous_iso_gv_spinning(Coords::Cartesian, 2, 2, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 2).is_ok(),
        "iso spinning kernel not JIT-able"
    );
}

#[test]
fn shaped_iso_porous_penalize_jit_compiles() {
    // the energy-free shaped wall (iso obstacle flows): same CSG normal, no nrg channel.
    let shape = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.5, 0.3, 0.2])
        .union(SdfExpr::sphere([0.6, 0.0, 0.0], 0.25));
    let program = penalize_porous_iso_gv_shaped(Coords::Cartesian, 2, 2, &shape, false);
    assert!(
        symbi_jit::compile_gv_kernel(&program, 2).is_ok(),
        "the iso shaped penalize kernel is outside the JIT subset",
    );
}

#[test]
fn shaped_penalize_jit_compiles_f32() {
    // the reduced-precision (f32) runtime-JIT path: the same CSG graph must cranelift-compile at
    // f32 (the f32 transcendental shims for tanh/sin/cos, f32 loads/consts/strides). covers the
    // CSG union (min/max kinks + Dual normal), the spinning kernel (f32 sin/cos shims), and the
    // iso curvilinear path (centroid_to_cartesian at f32).
    use symbi_ir::emit::Precision::F32;
    let csg = SdfExpr::<f64, 3>::cuboid([0.0, 0.0, 0.0], [0.5, 0.3, 0.2])
        .union(SdfExpr::sphere([0.6, 0.0, 0.0], 0.25));
    let program = penalize_porous_gv_shaped(Coords::Cartesian, 3, 3, &csg, false);
    assert!(
        symbi_jit::compile_gv_kernel_prec(&program, 3, F32).is_ok(),
        "f32 shaped kernel outside the JIT subset"
    );
    let program = penalize_porous_gv_spinning(Coords::Cartesian, 2, 2, &csg, false);
    assert!(
        symbi_jit::compile_gv_kernel_prec(&program, 2, F32).is_ok(),
        "f32 spinning shaped kernel not JIT-able"
    );
    let program = penalize_porous_iso_gv_shaped(Coords::Cylindrical, 2, 2, &csg, false);
    assert!(
        symbi_jit::compile_gv_kernel_prec(&program, 2, F32).is_ok(),
        "f32 iso cylindrical shaped kernel not JIT-able"
    );
}
