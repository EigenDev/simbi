...
impl<S: Scalar, const D: usize, E: EnergyModel> crate::state::SeedableCons<S, D>
    for MhdConsG<S, D, E>
{
    type Energy = E;
    #[inline]
    ...
    fn from_parts(hydro: ConsG<S, D, E>, mag: Option<Tensor<S, D>>) -> Self {
        MhdConsG {
            hydro,
            mag: mag.expect("MHD cons_at requires the magnetic field"),
        }
    }
}
...
impl<S: Scalar, const D: usize, E: EnergyModel> DerefMut for MhdPrimG<S, D, E> {
    #[inline]
    fn deref_mut(&mut self) -> &mut PrimG<S, D, E> {
        &mut self.hydro
    }
}
...
impl<S: Scalar, const D: usize, E: EnergyModel> DerefMut for MhdConsG<S, D, E> {
    #[inline]
    fn deref_mut(&mut self) -> &mut ConsG<S, D, E> {
        &mut self.hydro
    }
}
...
unsafe impl<const D: usize> FieldElement for MhdConsG<f64, D, Adiabatic> {
    type Scalar = f64;
}
unsafe impl<const D: usize> FieldElement for MhdConsG<f32, D, Adiabatic> {
    type Scalar = f32;
}
unsafe impl<const D: usize> FieldElement for MhdPrimG<f64, D, Adiabatic> {
    type Scalar = f64;
}
unsafe impl<const D: usize> FieldElement for MhdPrimG<f32, D, Adiabatic> {
    type Scalar = f32;
}

unsafe impl<const D: usize> FieldElement for MhdConsG<f64, D, IsoModel> {
    type Scalar = f64;
}
unsafe impl<const D: usize> FieldElement for MhdConsG<f32, D, IsoModel> {
    type Scalar = f32;
}
unsafe impl<const D: usize> FieldElement for MhdPrimG<f64, D, IsoModel> {
    type Scalar = f64;
}
unsafe impl<const D: usize> FieldElement for MhdPrimG<f32, D, IsoModel> {
    type Scalar = f32;
}
