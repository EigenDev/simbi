...
mod tests {
    ...
    fn flux_x_direction() {
        let regime = Newtonian;
        let eos = IdealGas { gamma: 1.4 };
        let prim = Prim { rho: 1.0, vel: Tensor::new([2.0, 0.0]), pre: 1.0 };
        let nhat = Tensor::unit(0); // x-direction
        let flux = regime.to_flux(&prim, &nhat, &eos);
    ...
    fn flux_y_direction() {
        let regime = Newtonian;
        let eos = IdealGas { gamma: 1.4 };
        let prim = Prim { rho: 1.0, vel: Tensor::new([2.0, 3.0]), pre: 1.0 };
    ...
    fn wave_speeds_at_rest() {
        let regime = Newtonian;
        let eos = IdealGas { gamma: 1.4 };
        let prim = Prim { rho: 1.0, vel: Tensor::new([0.0, 0.0]), pre: 1.0 };
    ...
    fn max_wave_speed_3d() {
        let regime = Newtonian;
        let eos = IdealGas { gamma: 5.0 / 3.0 };
        let prim = Prim { rho: 1.0, vel: Tensor::new([1.0, -2.0, 0.5]), pre: 1.0 };
    ...
    fn negative_pressure_flagged_with_raw_value() {
        ...
        let cons = Cons { den: 1.0, mom: Tensor::new([100.0]), nrg: 1.0 };
        let result = regime.to_primitive(&eos, &cons);
