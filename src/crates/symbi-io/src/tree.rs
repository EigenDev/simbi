...
impl DataRef<'_> {
    pub fn dtype(&self) -> DType {
        match self {
            Self::F64(_) => DType::F64, Self::F32(_) => DType::F32,
            Self::I64(_) => DType::I64, Self::U64(_) => DType::U64,
            Self::U8(_)  => DType::U8,
        }
    }
    pub fn len(&self) -> usize {
        match self {
            Self::F64(s) => s.len(), Self::F32(s) => s.len(),
            Self::I64(s) => s.len(), Self::U64(s) => s.len(),
            Self::U8(s)  => s.len(),
        }
    }
...
impl DataBuf {
    ...
    pub fn len(&self) -> usize {
        match self {
            Self::F64(v) => v.len(), Self::F32(v) => v.len(),
            Self::I64(v) => v.len(), Self::U64(v) => v.len(),
            Self::U8(v)  => v.len(),
        }
    }
    pub fn is_empty(&self) -> bool { self.len() == 0 }
...
impl<'a> Tree<'a> {
    pub fn new(name: impl Into<String>) -> Self {
        Self { name: name.into(), attrs: Vec::new(), datasets: Vec::new(), groups: Vec::new() }
    }
    ...
    pub fn push_dataset(&mut self, ds: Dataset<'a>) { self.datasets.push(ds); }
    /// in-place push: child group.
    pub fn push_group(&mut self, group: Tree<'a>) { self.groups.push(group); }

    /// merge a `Metadata` bag in one call.
    pub fn extend_attrs<'b>(&mut self, md: impl IntoIterator<Item = (&'b str, &'b Attr)>) {
        for (k, v) in md { self.attrs.push((k.to_string(), v.clone())); }
    }
    ...
    pub fn find_attr(&self, name: &str) -> Option<&Attr> {
        self.attrs.iter().find_map(|(k, v)| (k == name).then_some(v))
    }
