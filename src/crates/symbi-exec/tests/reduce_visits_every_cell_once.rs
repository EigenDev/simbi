...
use symbi_algebra::{domain, index, Domain};
...
fn big_box() -> Domain<3> {
    ...
    domain([index("i").over(67), index("j").over(23), index("k").over(43)])
}
...
fn parallel_reduce_visits_every_cell_exactly_once() {
    let d = big_box();
    assert!(d.volume() >= 1 << 16, "domain must exceed PAR_THRESHOLD to take the slab path");
    ...
    assert_eq!(field_reduce(&f, &d, ReductionOp::Max), (n - 1) as f64, "Max");
...
fn parallel_and_serial_folds_agree() {
    ...
    assert_eq!(got.to_bits(), want.to_bits(), "serial ADD reduce disagrees with a direct fold");
    ...
    assert_eq!(
        got_full.to_bits(), want_full.to_bits(),
