  ...
  public:
    ...
    DUAL mag_four_vec(const anyPrimitive<dim, Regime::RMHD>& prim)
        : lorentz(prim.lorentz_factor()),
          vdb(prim.vdotb()),
          zero(lorentz * vdb),
          one(prim.b1() / lorentz + lorentz * prim.proper_velocity(1) * vdb),
          two(prim.b2() / lorentz + lorentz * prim.proper_velocity(2) * vdb),
          three(prim.b3() / lorentz + lorentz * prim.proper_velocity(3) * vdb)
    {
    }
...
struct anyPrimitive : generic_hydro::anyHydro<dim, anyPrimitive<dim, R>, R> {
    ...
    DUAL constexpr real proper_velocity(1) const
    {
    ...
    DUAL constexpr real proper_velocity(2) const
    {
        if constexpr (dim > 1) {
            if constexpr (R == Regime::SRHD || R == Regime::RMHD) {
                if constexpr (global::VelocityType == global::Velocity::Beta) {
    ...
    DUAL constexpr real proper_velocity(3) const
    {
        if constexpr (dim > 2) {
            if constexpr (R == Regime::SRHD || R == Regime::RMHD) {
                if constexpr (global::VelocityType == global::Velocity::Beta) {
    ...
    DUAL anyConserved<dim, R> to_flux(const real gamma, const luint nhat) const
    {
        if constexpr (R == Regime::NEWTONIAN) {
            ...
            return {
              d * vn,
              m1 * vn + kdelta(nhat, 1) * ptot - bn * bmu[0] * invlf,
              m2 * vn + kdelta(nhat, 2) * ptot - bn * bmu[1] * invlf,
              m3 * vn + kdelta(nhat, 3) * ptot - bn * bmu[3] * invlf,
              mn - d * vn,
              ind1,
              ind2,
              ind3,
