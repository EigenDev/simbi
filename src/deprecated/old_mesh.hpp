...
namespace simbi {
    ...
    struct Mesh {
        ...
        {
            switch (cellspacing) {
                case simbi::Cellspacing::LOGSPACE: return std::sqrt(a * b);
                default: return 0.5 * (a + b);
            }
        }
        ...
        struct CellParams {
            ...
            DUAL constexpr real get_x2_differential(const lint ii) const
            {
                if constexpr (dim == 1) {
                    switch (parent.derived().geometry) {
                        case Geometry::SPHERICAL: return 2.0;
                        default: return (2.0 * M_PI);
                    }
                }
                else {
                    switch (parent.derived().geometry) {
            ...
            DUAL constexpr real get_x3_differential(const lint ii) const
            {
                if constexpr (dim == 1) {
                    switch (parent.derived().geometry) {
                        case Geometry::SPHERICAL: return (2.0 * M_PI);
                        default: return 1.0;
                    }
                }
                else if constexpr (dim == 2) {
                    switch (parent.derived().geometry) {
                        case Geometry::PLANAR_CYLINDRICAL: return 1.0;
                        default: return (2.0 * M_PI);
                    }
                }
                else {
            ...
            DUAL auto geometrical_sources_spherical_rmhd(const auto& prb) const
            {
                ...
                const real v1    = prb.proper_velocity(1);
                const real v2    = prb.proper_velocity(2);
                const real v3    = prb.proper_velocity(3);
                ...
                const auto rsource =
                    ...
                    (bmu[1] * bmu[1] + bmu[3] * bmu[3]) / x1mean;

                const auto tsource =
                    pt * (a2R() - a2L()) / dV -
                    wgam2 * (v2 * v1 - v3 * v3 * cot) / x1mean +
                    (bmu[1] * bmu[0] - bmu[3] * bmu[3] * cot) / x1mean;
                const auto psource = -wgam2 * v3 * (v1 + cot * v2) / x1mean +
                                     bmu[3] * (bmu[0] + cot * bmu[1]) / x1mean;
            ...
            ) const
            {
                ...
                const real rsource =
                    (wgam2 * v2 * v2 - bmuc[2] * bmuc[2] + pt) / x1mean;
                const real psource =
                    -(wgam2 * v1 * v2 - bmuc.one * bmuc[2]) / x1mean;
