...
#include "core/containers/array_view.hpp"      // for array_view"
...
namespace simbi {
    ...
    class ndarray : public array_properties<T, Dims>
    {
      public:
        ...
        ndarray(
            T* external_data,
            const collapsable_t<size_type, Dims>& shape,
            bool take_ownership
        )
        {
            // set up metadata
        ...
        {
            collapsable_t<size_type, Dims> idx{
        ...
        auto& reshape(const collapsable_t<size_type, Dims>& new_shape)
        {
        ...
        auto& reshape(
            const collapsable_t<size_type, Dims>& new_shape,
            const collapsable_t<size_type, Dims>& new_strides
        )
        {
            // Verify total size matches
            assert(
        ...
        auto contract(const collapsable_t<size_type, Dims>& radii)
            -> array_view<T, Dims>
        {
            uarray<Dims> new_shape;
            uarray<Dims> offsets;
        ...
        auto view(const collapsable_t<T, Dims>& ranges) const
            -> array_view<T, Dims>
        {
            uarray<Dims> offsets;
            for (size_type ii = 0; ii < Dims; ++ii) {
        ...
        template <typename F>
        ndarray_t<size_type, 1>
        filter_indices(F op, const ExecutionPolicy<>& policy) const
        {
            ndarray_t<size_type, 1> indices(this->size());
