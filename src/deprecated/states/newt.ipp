...
) const
{
    ...
    auto fL = prL.to_flux(gamma, unit_vectors::canonical_basis<dim>(nhat));
    auto fR = prR.to_flux(gamma, unit_vectors::canonical_basis<dim>(nhat));
...
) const
{
    ...
    if constexpr (dim == 1) {
        ...
        const auto ehat = unit_vectors::canonical_basis<dim>(nhat);
        ...
        const real phi = EntropyDetector::compute_adaptive_phi(
            prL,
            prR,
            unit_vectors::canonical_basis<dim>(nhat),
            gamma,
