...
#include "core/utility/atomic_bool.hpp"
...
) const
{
    ...
    const auto fL =
        prL.to_flux(gamma, unit_vectors::canonical_basis<dim>(nhat));
    const auto fR =
        prR.to_flux(gamma, unit_vectors::canonical_basis<dim>(nhat));
...
) const -> std::pair<real, real>
{
    ...
    const auto snorm =
        vecops::dot(uhlls, unit_vectors::canonical_basis<dim>(nhat));
    const auto fe = fhlltau + fhlld;
    const auto fsnorm =
        vecops::dot(fhlls, unit_vectors::canonical_basis<dim>(nhat));
