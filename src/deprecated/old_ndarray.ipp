...
template <typename DT, int dim>
constexpr simbi::ndarray<DT, dim>& simbi::ndarray<DT, dim>::resize(
    size_type new_size,
    const StrideInfo& new_strides
)
{
    if (new_size > sz) {
...
template <typename DT, int dim>
void simbi::ndarray<DT, dim>::sync_to_device
{
...
template <typename DT, int dim>
void simbi::ndarray<DT, dim>::sync_to_host()
{
    if (dev_arr) {
        gpu::api::copyDeviceToHost(arr.get(), dev_arr.get(), sz * sizeof(DT));
    }
}
...
) const
{
    if constexpr (global::on_gpu) {
        ...
        parallel_for(policy, [=] DEV(size_type bid) {
            ...
            __shared__ U shared[256];

            // Local reduction
            U thread_sum = init;

            // Grid-stride loop
            for (size_type ii = bid * threads_per_block + tid; ii < num_items;
                 ii += grid_stride) {
                thread_sum = binary_op(thread_sum, in_ptr[ii], ii);
            }
            ...
            for (int s = threads_per_block / 2; s > 0; s >>= 1) {
                if (tid < s) {
                    shared[tid] = binary_op(shared[tid], shared[tid + s], tid);
                }
                gpu::api::synchronize();
            }
        ...
        const size_type num_batches = policy.get_num_batches(sz);
        ...
        parallel_for(policy, num_batches, [&](size_type bid) {
            ...
            for (size_type i = start; i < end; i++) {
                local_result = binary_op(local_result, arr[i], i);
            }

            // Atomic reduction using actual array element
            U current_min = result.load();
            while (true) {
                U next_min = binary_op(current_min, arr[start], start);
...
//             if (lane == 0) {
//                 result_ptr[lane] = gpu::api::atomicMin(result_ptr,
//                 thread_sum);
//             }
//         });
...
template <typename DT, int dim>
template <typename Func>
auto simbi::ndarray<DT, dim>::transform(
    const ExecutionPolicy<>& policy,
    Func transform_op
) const
    -> std::enable_if_t<
...
simbi::ndarray<DT, dim>::transform_with(
    const ExecutionPolicy<>& policy,
...
// template <typename DT, int dim>
// template <typename StencilArray, typename Func>
// void simbi::ndarray<DT, dim>::transform_stencil_with(
//     const ExecutionPolicy<>& policy,
//     const StencilArray& stencil_array,
//     size_type radius,
//     Func stencil_op
// )
// {
...
//             stencil_view<T> view(stencil_ptr, sx, sy, sz, ii, jj, kk,
//             radius);
...
template <typename DT, int dim>
template <typename MatchingStencil, typename... DependentArrays, typename Func>
void simbi::ndarray<DT, dim>::transform_stencil_with(
    const ExecutionPolicy<>& policy,
    size_type radius,
    Func stencil_op,
    const MatchingStencil& primary_stencil,
...
)
{
    ...
    if constexpr (global::on_gpu) {
        auto data_ptr     = dev_data();
        auto stencil_ptrs = std::make_tuple(dependent_arrays.dev_data()...);
        ...
        parallel_for(policy, [=] DEV(size_type idx) {
            ...
            // Create stencil view of input arrays
            // auto stencil_views = std::make_tuple(
            //     stencil_view<typename
            //     array_value_type<DependentArrays>::type>(
            //         std::get<idx>(stencil_ptrs),
            //         sx,
            //         sy,
            //         sz,
            //         ii,
            ...
            // Update array in-place
            // data_ptr[idx3(ii, jj, kk, nxf, nyf, nzf)] +=
            //     stencil_op(stencil_views...);
        });
    }
    else {
        parallel_for(policy, [&](size_type idx) {
            ...
            auto stencil_views = std::make_tuple(
                stencil_vew<typename array_value_type<MatchingStencil>::type>(
                    primary_stencil.data(),
                    primary_stencil.get_strides().x,
                    primary_stencil.get_strides().y,
                    primary_stencil.get_strides().z,
                    ii,
                    jj,
                    kk
                ),
                stencil_view<typename array_value_type<DependentArrays>::type>(
                    dependent_arrays.data(),
                    dependent_arrays.get_strides().x,
                    dependent_arrays.get_strides().y,
                    dependent_arrays.get_strides().z,
                    ii,
                    jj,
                    kk
                )...
            ...
            arr[central_idx] += std::apply(stencil_op, stencil_views);
        });
    }
}
...
void simbi::ndarray<DT, dim>::apply_to_corners(
    const ExecutionPolicy<>& policy,
    size_type radius,
...
template <typename DT, int dim>
template <typename T, typename Func>
void simbi::ndarray<DT, dim>::fold(
    const ExecutionPolicy<>& policy,
    simbi::ndarray<T, dim>& output,
    Func fold_op
) const
{
    if constexpr (global::on_gpu) {
        auto in_ptr  = dev_data();
        auto out_ptr = output.dev_data();

        parallel_for(policy, size(), [=] DEV(size_type idx) {
            out_ptr[idx] = fold_op(in_ptr[idx]);
        });
    }
    else {
        parallel_for(policy, size(), [&](size_type idx) {
            output[idx] = fold_op(arr[idx]);
        });
    }
}
...
template <typename DT, int dim>
template <typename T, typename Func>
simbi::ndarray<T> simbi::ndarray<DT, dim>::fold_into(
    const ExecutionPolicy<>& policy,
    simbi::ndarray<T>& output,
    Func fold_op
) const
{
    if constexpr (global::on_gpu) {
        ...
        parallel_for(policy, size(), [=] DEV(size_type idx) {
            out_ptr[idx] = fold_op(in_ptr[idx], idx);
        });
    }
    else {
        parallel_for(policy, size(), [&](size_type idx) {
            output[idx] = fold_op(arr[idx], idx);
        });
    }
    return output;
}
...
void simbi::ndarray<DT, dim>::fold_with(
    const ExecutionPolicy<>& policy,
    simbi::ndarray<T, dim>& output,
    simbi::ndarray<U>& other,
    Func fold_op
) const
{
    if constexpr (global::on_gpu) {
        ...
        parallel_for(policy, size(), [=] DEV(size_type idx) {
            out_ptr[idx] = fold_op(in_ptr[idx], other_ptr[idx]);
        });
    }
    else {
        parallel_for(policy, size(), [&](size_type idx) {
            output[idx] = fold_op(arr[idx], other[idx]);
        });
    }
}