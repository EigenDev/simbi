#ifndef HET_MEM_VIEW_HPP
#define HET_MEM_VIEW_HPP
...
namespace simbi::het::mem {
    ...
    // now uses explicit strides for correct multi-dimensional access
    template <typename T, std::uint64_t Rank = 1>
    struct view_t {
        static_assert(
            std::is_trivially_copyable_v<T>,
            "view types must be trivial"
        );
        ...
        {
            auto idx = vecops::dot(coord - start_, strides_);
            return data_[idx];
        }
        ...
        DUAL T* data() const { return data_; }

        // helper for bounds
        DUAL std::uint64_t size() const { return fp::product(shape_); }
        DUAL iarray<Rank> shape() const { return shape_; }
    };

}   // namespace simbi::het::mem

#endif   // HETERO_MEM_VIEW_HPP
