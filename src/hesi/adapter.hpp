...
namespace simbi::het {
    ...
    namespace info {
        ...
        static constexpr auto backend_name = backend_traits<default_backend_t>::name;

        // is this a gpu backend?
        static constexpr bool is_gpu = is_gpu_backend_v<default_backend_t>;

        // warp/wavefront size
        static constexpr int warp_size = backend_traits<default_backend_t>::warp_size;
        ...
        inline device_info_t query_device(std::int64_t device_id)
        {
            device_info_t info;
    ...
    namespace config {
        ...
        inline void set_device(std::int64_t device_id)
        {
#if defined(CUDA_ENABLED)
    ...
    inline stream_t make_stream(backend_type_t backend, std::int64_t device_id = 0)
    {
