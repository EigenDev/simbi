...
namespace simbi {
    ...
    namespace vecops {
            ...
            requires(Vec1::dimensions == 3 && Vec2::dimensions == 3)
        {
            ...
            return vector_t<T, 3, ResultType>(
                a[1] * b[2] - a[2] * b[1],
                a[2] * b[0] - a[0] * b[2],
                a[0] * b[1] - b[0] * a[1]
            );
            ...
            requires Magnetic<Type>
        {
            ...
            return vector_t<T, 4, VectorType::MAGNETIC_FOUR>{
              b0,
              bs[0],
              bs[1],
              bs[2]
        ...
        {
            return vector_t<T, 2, VectorType::SPATIAL>{
    ...
    class vector_t
    {
      ...
      public:
        ...
        DUAL constexpr explicit vector_t(Args&&... args)
            requires(
                sizeof...(Args) == Dims &&
        ...
        DUAL constexpr explicit vector_t(const array_t<T, Dims>& arr)
            : storage_(arr)
        {
        }

        // construction from raw pointer
        DUAL constexpr explicit vector_t(const T* data)
        {
        ...
        DUAL constexpr auto
        operator+=(const vector_t<U, OtherDims, OtherType>& other)
        {
            // traditional for loop version
            // for (size_type ii = 0; ii < Dims; ++ii) {
        ...
        DUAL constexpr vector_view_t<T, Dims, Type> view()
        {
            return vector_view_t<T, Dims, Type>(storage_.data());
        }

        DUAL constexpr vector_view_t<const T, Dims, Type> view() const
        {
            return vector_view_t<const T, Dims, Type>(storage_.data());
        }
        ...
        {
            return vector_t<T, 3, VectorType::MAGNETIC>{storage_.data() + 1};
        }
        ...
        {
            return vector_t<T, Dims, NewType>(storage_);
        }

        DUAL constexpr auto as_general() const
        {
            return vector_t<T, Dims, VectorType::GENERAL>(storage_);
        }

        DUAL constexpr auto as_magnetic() const
        {
            return vector_t<T, Dims, VectorType::MAGNETIC>(storage_);
        }
    ...
    // -------------------------------------------------------------
    // vector_view_t: non-owning view of vector data
    // -------------------------------------------------------------
    template <typename T, size_type Dims, VectorType Type = VectorType::GENERAL>
    class vector_view_t
    {
      ...
      public:
        ...
        DUAL constexpr explicit vector_view_t(T* data = nullptr) : data_(data)
        {
        }
        ...
        DUAL constexpr vector_t<std::remove_const_t<T>, Dims, Type>
        to_vector() const
        {
            return vector_t<std::remove_const_t<T>, Dims, Type>(data_);
        }
        ...
        // implicit conversion to owning Vector
        DUAL constexpr
        operator vector_t<std::remove_const_t<T>, Dims, Type>() const
        {
            return vector_t<std::remove_const_t<T>, Dims, Type>(data_);
        }
    };
    ...
    //---------------------------------------------------------------
    // Constvector_view_t: const version of vector_view_t
    // ---------------------------------------------------------------
    template <typename T, size_type Dims, VectorType Type = VectorType::GENERAL>
    class Constvector_view_t
    {
      private:
      ...
      public:
        ...
        // constructor from pointer
        DUAL constexpr explicit Constvector_view_t(const T* data) : data_(data)
        {
        }

        // brace-enclosed initializer list constructor
        DUAL constexpr explicit Constvector_view_t(
        ...
        // convert to owning Vector
        DUAL constexpr vector_t<T, Dims, Type> to_vector() const
        {
            return vector_t<T, Dims, Type>(data_);
        }
        ...
        // implicit conversion to owning Vector
        DUAL constexpr operator vector_t<T, Dims, Type>() const
        {
            return vector_t<T, Dims, Type>(data_);
        }
    };
        ...
        requires(Vec1::dimensions == Vec2::dimensions)
    {
        ...
        vector_t<result_t, Dims, ResultType> result;
    ...
    using spatial_vector_t = vector_t<T, Dims, VectorType::SPATIAL>;
    template <typename T, size_type Dims>
    using magnetic_vector_t = vector_t<T, Dims, VectorType::MAGNETIC>;
    template <typename T, size_type Dims>
    using general_vector_t = vector_t<T, Dims, VectorType::GENERAL>;

    template <typename T>
    using magnetic_four_vector_t = vector_t<T, 4, VectorType::MAGNETIC_FOUR>;
    template <typename T>
    using spacetime_vector_t = vector_t<T, 4, VectorType::SPACETIME>;
    // unit vector type
    template <size_type Dims>
    using unit_vector_t = vector_t<luint, Dims, VectorType::SPATIAL>;

    // view aliases
    template <typename T, size_type Dims>
    using spatial_vector_view_t = vector_view_t<T, Dims, VectorType::SPATIAL>;
    template <typename T, size_type Dims>
    using magnetic_vector_view_t = vector_view_t<T, Dims, VectorType::MAGNETIC>;
    template <typename T, size_type Dims>
    using general_vector_view_t = vector_view_t<T, Dims, VectorType::GENERAL>;

    template <typename T>
    using spacetime_vector_view_t = vector_view_t<T, 4, VectorType::SPACETIME>;

    // const aliases
    template <typename T, size_type Dims>
    using const_spatial_vector_view_t =
        Constvector_view_t<T, Dims, VectorType::SPATIAL>;
    template <typename T, size_type Dims>
    using const_magnetic_vector_view_t =
        Constvector_view_t<T, Dims, VectorType::MAGNETIC>;
    template <typename T>
    using const_magnetic_four_vector_view_t =
        Constvector_view_t<T, 4, VectorType::MAGNETIC_FOUR>;

    // zero vector view as a global singleton
    template <typename T, size_type Dims, VectorType Type>
    class Zerovector_view_t
    {
      private:
      ...
      public:
        DUAL constexpr vector_view_t<const T, Dims, Type> view() const
        {
            return vector_view_t<const T, Dims, Type>(zero_storage_.data());
        }
    };

    class ZeroMagneticvector_view_t
        : public const_magnetic_vector_view_t<real, 3>
    {
      private:
        const real zero_storage[3] = {0.0, 0.0, 0.0};

      public:
        DUAL ZeroMagneticvector_view_t()
            : const_magnetic_vector_view_t<real, 3>(zero_storage)
        {
        }
    };

    class ZeroMagneticFourvector_view_t
        : public const_magnetic_four_vector_view_t<real>
    {
      ...
      public:
        DUAL ZeroMagneticFourvector_view_t()
    ...
    DUAL constexpr auto operator|(const vector_t<T, Dims, Type>& v, F&& f)
    ...
    namespace unit_vectors {
        template <size_type Dims>
        DUAL constexpr auto canonical_basis(size_type i)
        {
            if constexpr (Dims == 1) {
                // Return a vector directly instead of referencing static data
                return (i == 1) ? vector_t<luint, 1, VectorType::SPATIAL>{1}
                                : vector_t<luint, 1, VectorType::SPATIAL>{0};
            }
            else if constexpr (Dims == 2) {
                if (i == 1) {
                    return vector_t<luint, 2, VectorType::SPATIAL>{1, 0};
                }
                else if (i == 2) {
                    return vector_t<luint, 2, VectorType::SPATIAL>{0, 1};
                }
                else {
                    return vector_t<luint, 2, VectorType::SPATIAL>{0, 0};
                }
            }
            else {
                if (i == 1) {
                    return vector_t<luint, 3, VectorType::SPATIAL>{1, 0, 0};
                }
                else if (i == 2) {
                    return vector_t<luint, 3, VectorType::SPATIAL>{0, 1, 0};
                }
                else if (i == 3) {
                    return vector_t<luint, 3, VectorType::SPATIAL>{0, 0, 1};
                }
                else {
                    return vector_t<luint, 3, VectorType::SPATIAL>{0, 0, 0};
                }
            }
        }
    }   // namespace unit_vectors
    ...
    template <typename T, size_type Dims, VectorType Type>
    DUAL std::ostream&
    operator<<(std::ostream& os, const vector_t<T, Dims, Type>& v)
    {
        os << "[";
    ...
    template <typename T, size_type Dims, VectorType Type>
    DUAL std::ostream&
    operator<<(std::ostream& os, const Constvector_view_t<T, Dims, Type>& v)
    {
        os << "[";
        for (size_type i = 0; i < Dims; ++i) {
            os << v[i];
...
namespace std {
    ...
    struct tuple_size<simbi::vector_t<T, Dims, Type>>
        : integral_constant<size_t, Dims> {
    };

    template <size_t I, typename T, size_type Dims, simbi::VectorType Type>
    struct tuple_element<I, simbi::vector_t<T, Dims, Type>> {
        using type = T;
    };

    template <size_t I, typename T, size_type Dims, simbi::VectorType Type>
    DUAL constexpr T& get(simbi::vector_t<T, Dims, Type>& v)
    {
    ...
    template <size_t I, typename T, size_type Dims, simbi::VectorType Type>
    DUAL constexpr const T& get(const simbi::vector_t<T, Dims, Type>& v)
    {
        static_assert(I < Dims, "index out of bounds");
        return v[I];
    }
