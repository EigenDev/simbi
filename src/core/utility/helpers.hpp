...
namespace simbi::helpers {
    ...
    */
            ...
            if constexpr (is_mhd_c<prim_t>) {
                ...
                switch (comp_slope_limiter) {
                    case LIMITER::VAN_LEER:
                        result[qq] = vanLeer(pr[qq] - pc[qq], pc[qq] - pl[qq]);
                        break;
                    default:
                        result[qq] = minmod(
                            (pc[qq] - pl[qq]) * plm_theta,
                            (pr[qq] - pl[qq]) * 0.5,
                            (pr[qq] - pc[qq]) * plm_theta
                        );
                        break;
                }
            }
    ...
    template <typename T>
    DUAL std::int64_t solve_quartic(T b, T c, T d, T e, T res[4]);
    ...
    {
        for_each_index(std::forward<F>(func), std::make_index_sequence<N>{});
    }
