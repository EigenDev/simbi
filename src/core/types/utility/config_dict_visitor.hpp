...
namespace simbi {
    ...
    /**
     * @brief Visitor that populates InitialConditions from a ConfigDict
     */
    class ConfigDictVisitor : public InitialConditionsVisitor
    {
      private:
        const ConfigDict& dict;

      public:
        explicit ConfigDictVisitor(const ConfigDict& config) : dict(config) {}
        ...
        void visit_resolution(luint& nx, luint& ny, luint& nz) override
        {
            const auto nghosts =
                2 *
                (1 + (dict.at("spatial_order").get<std::string>() == "plm"));
        ...
        ) override
        {
            ...
            spatial_order  = dict.at("spatial_order").get<std::string>();
            temporal_order = dict.at("temporal_order").get<std::string>();
        ...
        visit_immersed_bodies(std::vector<ConfigDict>& immersed_bodies) override
        {
            if (dict.contains("immersed_bodies") &&
                ...
                const auto& body_list =
                    dict.at("immersed_bodies").get<std::list<ConfigDict>>();
