...
typedef sr1d::PrimitiveArray PrimitiveArray;
typedef sr1d::ConservedArray ConservedArray;
    ...
    const Primitive &right_prims)
{
    ...
    hll_states.D = (aR * right_state.D - aL * left_state.D - right_flux.D + left_flux.D) / (aR - aL);

    hll_states.S = (aR * right_state.S - aL * left_state.S - right_flux.S + left_flux.S) / (aR - aL);

    hll_states.tau = (aR * right_state.tau - aL * left_state.tau - right_flux.tau + left_flux.tau) / (aR - aL);
...
Conserved SRHD::calc_hll_flux(const Primitive &left_prims,
                              const Primitive &right_prims,
                              const Conserved &left_state,
                              const Conserved &right_state,
                              const Conserved &left_flux,
                              const Conserved &right_flux)
{
    ...
    hll_flux.D = ((aRp * left_flux.D - aLm * right_flux.D + aLm * aRp * (right_state.D - left_state.D)) /
                  (aRp - aLm));

    hll_flux.S = (aRp * left_flux.S - aLm * right_flux.S + aLm * aRp * (right_state.S - left_state.S)) /
                 (aRp - aLm);

    hll_flux.tau = (aRp * left_flux.tau - aLm * right_flux.tau + aLm * aRp * (right_state.tau - left_state.tau)) /
                   (aRp - aLm);
                                        ...
                                        bool hllc = false)
{
    ...
    double t_interval = t == 0 ? floor(tstart * round_place + 0.5) / round_place : floor(tstart * round_place + 0.5) / round_place + chkpt_interval;
