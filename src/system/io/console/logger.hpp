        ...
        namespace logger {
            ...
            class CursorManager
            {
              public:
                ...
                static void signalHandler(std::int64_t signal)
                {
                    // Show the cursor
                    std::cout << "\033[?25h";
                    // Exit the program
            ...
            struct Logger {
                ...
                {
                    auto unwravel_idx = [&](std::uint64_t idx,
                                            const std::vector<std::uint64_t>&
                                                shape) {
                        std::vector<std::uint64_t> coords(shape.size());

                        if (global::col_major) {
                            // Column-major order (Fortran-style)
                            std::uint64_t stride = 1;
                            for (std::uint64_t ii = 0; ii < shape.size();
                                 ++ii) {
                            ...
                            std::uint64_t stride = 1;
                            for (std::int64_t ii = shape.size() - 1; ii >= 0;
                                 --ii) {
                                coords[ii] = (idx / stride) % shape[ii];
                                stride *= shape[ii];
                            }
                ...
                // Prstd::int64_t the benchmark results
                template <typename sim_state_t>
            ...
            void with_logger(sim_state_t& sim_state, real end_time, F&& f)
            {
                ...
                // use pretty table to prstd::int64_t the results
                ...
                // prstd::int64_t the initial table
                table.print();
                ...
                while ((sim_state.time() < end_time) &&
                    ...
                    try {
                        ...
                        // Write to a file at every checkpostd::int64_t interval
