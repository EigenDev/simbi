...
namespace simbi::expression {
    ...
    )
    {
        ...
        for (std::int64_t node_idx : eval_order) {
            ...
            switch (node.op) {
                case ExprOp::CONSTANT:
                    instr.payload.constant_eval = node.data.value;
                    break;
                ...
                case ExprOp::VARIABLE_T:
                    ...
                    instr.payload.register_operands.operand1 = op2reg(node.op);
                    break;

                case ExprOp::PARAMETER:
                    instr.op                    = ExprOp::PARAMETER;
                    instr.payload.parameter_idx = node.data.param_idx;
                    break;

                case ExprOp::IF_THEN_ELSE:
                    instr.payload.register_operands.operand1 =
                        node_to_reg[node.data.ternary.condition];
                    instr.payload.register_operands.operand2 =
                        node_to_reg[node.data.ternary.then_expr];
                    instr.payload.register_operands.operand3 =
                        node_to_reg[node.data.ternary.else_expr];
                    break;

                default:
                    // Binary and unary operations
                    if (node.data.children.left >= 0) {
                        instr.payload.register_operands.operand1 =
                            node_to_reg[node.data.children.left];
                    }
                    else {
                        instr.payload.register_operands.operand1 = -1;
                    }

                    if (node.data.children.right >= 0) {
                        instr.payload.register_operands.operand2 =
                            node_to_reg[node.data.children.right];
                    }
                    else {
                        instr.payload.register_operands.operand2 = -1;
                    }

                    instr.payload.register_operands.operand3 = -1;
                    break;
            }
        ...
        store_t<std::int64_t>    mapped_output_indices(output_indices.size());
        store_t<LinearExprInstr> result_instrs(instructions.size());
    ...
    )
    {
        ...
        while (!to_process.empty()) {
            ...
            switch (node.op) {
                case ExprOp::IF_THEN_ELSE:
                    to_process.push(node.data.ternary.condition);
                    to_process.push(node.data.ternary.then_expr);
                    to_process.push(node.data.ternary.else_expr);
                    break;
                ...
                default:
                    // binary and unary operations
                    if (node.data.children.left >= 0) {
                        to_process.push(node.data.children.left);
                    }
                    if (node.data.children.right >= 0) {
                        to_process.push(node.data.children.right);
                    }
                    break;
            }
        }
        ...
        std::function<void(int)> visit = [&](std::int64_t node_id) {
            ...
            if (it != visited.end()) {
                if (it->second == 1) {
                    ...
                    throw std::runtime_error("Cycle detected in expression graph");
                }
                else if (it->second == 2) {
            ...
            switch (node.op) {
                case ExprOp::IF_THEN_ELSE:
                    visit(node.data.ternary.condition);
                    visit(node.data.ternary.then_expr);
                    visit(node.data.ternary.else_expr);
                    break;
                ...
                default:
                    // binary and unary operations
                    if (node.data.children.left >= 0) {
                        visit(node.data.children.left);
                    }
                    if (node.data.children.right >= 0) {
                        visit(node.data.children.right);
                    }
                    break;
            }
