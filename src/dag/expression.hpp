...
namespace simbi::expression {
    ...
    struct ExprNode
    {
        ...
        static ExprNode unary(ExprOp op, std::int64_t left)
        {
            ...
            node.data.children.right = -1; // Unused
            return node;
        }
        ...
        static ExprNode
        from_ternary(std::int64_t cond, std::int64_t then_branch, std::int64_t else_branch)
        {
            ExprNode node;
            node.op                     = ExprOp::IF_THEN_ELSE;
            node.data.ternary.condition = cond;
            node.data.ternary.then_expr = then_branch;
            node.data.ternary.else_expr = else_branch;
            return node;
        }
    };
