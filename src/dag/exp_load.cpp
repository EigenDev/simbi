...
namespace simbi::expression {
    ...
    ExprOp string_to_expr_op(const std::string& op_str)
    {
        ...
        std::cerr << "Unknown operation: " << op_str << ". Defaulting to CONSTANT." << std::endl;
    ...
    store_t<ExprNode> load_expressions(const config_dict_t& expr_data)
    {
        ...
        if (!expr_data.contains("expressions") || !expr_data.at("expressions").is_list()) {
            return nodes;
        }

        const auto& expressions_list =
            expr_data.at("expressions").template get<std::list<config_dict_t>>();
        ...
        for (auto& expr_node : expressions_list) {
            ...
            if (expr_node.contains("op") && expr_node.at("op").is_string()) {
                std::string op_str = expr_node.at("op").template get<std::string>();
    ...
    store_t<std::int64_t> get_output_indices(const config_dict_t& expr_data)
    {
        ...
        store_t res(expr_data.at("output_indices").template get<std::vector<std::int64_t>>());
        return res;
    }
    ...
    store_t<real> get_parameters(const config_dict_t& expr_data)
    {
        ...
        store_t res(expr_data.at("parameters").template get<std::vector<real>>());
        return res;
    }
    ...
    store_t<real> get_parameter_range(const config_dict_t& expr_data)
    {
        ...
        auto res = store_t<real>(expr_data.at("param_count").template get<std::int64_t>());
        return res;
    }
    ...
    {
        return {load_expressions(data), get_output_indices(data), get_parameter_range(data)};
    }
} // namespace simbi::expression
