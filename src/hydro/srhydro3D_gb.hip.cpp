    ...
    simbi::parallel_for(p, (luint)0, nzones, [=] GPU_LAMBDA (luint gid){
        ...
        while (!found_failure && workLeftToDo)
        {
            ...
            real v3 = S3 * inv_et;
                ...
                self->gpu_prims[gid]          = Primitive{rho, W * v1, W * v2, W * v3, peq};
            #else
                pressure_guess[gid] = peq;
                prims[gid]          = Primitive{rho, W * v1, W * v2, W * v3,  peq};
            #endif
            workLeftToDo = false;
        }
    });
    ...
    const luint nhat)
{
    ...
    const real uL      = primsL.vcomponent(nhat);
    const real uR      = primsR.vcomponent(nhat);
    const real lorentz = std::sqrt(1 + (uL * uL + uR * uR));
    const real vL      = uL / lorentz;
    const real vR      = uR / lorentz;
...
Conserved SRHD3D::prims2cons(const Primitive &prims)
{
    ...
    const real u1 = prims.v1;
    const real u2 = prims.v2;
    const real u3 = prims.v3;
    const real pressure = prims.p;
    const real lorentz_gamma = std::sqrt(static_cast<real>(1.0) + (u1 * u1 + u2 * u2 + u3 * u3));
    const real h = static_cast<real>(1.0) + gamma * pressure / (rho * (gamma - 1));

    return Conserved{
        rho * lorentz_gamma, 
        rho * h * lorentz_gamma * u1,
        rho * h * lorentz_gamma * u2,
        rho * h * lorentz_gamma * u3,
            ...
            for (luint jj = 0; jj < yphysical_grid; jj++)
            {
                ...
                for (luint ii = 0; ii < xphysical_grid; ii++)
                {
                    ...
                    const auto aid      = shift_k * nx * ny + shift_j * nx + shift_i;
                          real rho      = prims[aid].rho;
                          real v1       = prims[aid].v1;
                          real v2       = prims[aid].v2;
                          real v3       = prims[aid].v3;
                    const real lorentz  = std::sqrt(1 + (v1 * v2 + v2 * v2 + v3 * v3));
                    v1 /= lorentz;
                    v2 /= lorentz;
                    v3 /= lorentz;
...
Conserved SRHD3D::calc_Flux(const Primitive &prims, const luint nhat = 1)
{
    ...
    const real u3       = prims.v3;
    const real lorentz_gamma = std::sqrt(static_cast<real>(1.0) + (u1 * u1 + u2 * u2 + u3 * u3));
    const real v1 = u1 / lorentz_gamma;
    const real v2 = u2 / lorentz_gamma;
    const real v3 = u3 / lorentz_gamma;
    const real pressure = prims.p;

    const real h  = static_cast<real>(1.0) + gamma * pressure / (rho * (gamma - static_cast<real>(1.0)));
    const real D  = rho * lorentz_gamma;
    const real S1 = rho * lorentz_gamma * u1 * h;
    const real S2 = rho * lorentz_gamma * u2 * h;
    const real S3 = rho * lorentz_gamma * u3 * h;
    const real tau =
                    rho * h * lorentz_gamma * lorentz_gamma - pressure - rho * lorentz_gamma;
    ...
    const   luint nhat = 1)
{
    ...
    if (-aL <= (aStar - aL))
    {
        ...
        const real un = left_prims.vcomponent(nhat);
        const real lorentz = left_prims.calc_lorentz_gamma();
        const real vn = un / lorentz;
        // Left Star State in x-direction of coordinate lattice
        const real Dstar    = cofactor * (aL - vn) * D;
        const real S1star   = cofactor * (S1 * (aL - vn) - pressure + pStar);
        const real S2star   = cofactor * (aL - vn) * S2;
        const real S3star   = cofactor * (aL - vn) * S3;
        const real Estar    = cofactor * (E * (aL - vn) + pStar * aStar - pressure * vn);
    ...
    else
    {
        ...
        const real un      = right_prims.vcomponent(nhat);
        const real lorentz = right_prims.calc_lorentz_gamma();
        const real vn      = un / lorentz;
        // Left Star State in x-direction of coordinate lattice
        const real Dstar    = cofactor * (aR - vn) * D;
        const real S1star   = cofactor * (S1 * (aR - vn) - pressure + pStar);
        const real S2star   = cofactor * (aR - vn) * S2;
        const real S3star   = cofactor * (aR - vn) * S3;
        const real Estar    = cofactor * (E * (aR - vn) + pStar * aStar - pressure * vn);
        const real tauStar  = Estar - Dstar;
    ...
    const simbi::MemSide user)
{
    ...
    simbi::parallel_for(p, (luint)0, extent, [=] GPU_LAMBDA (const luint idx){
        ...
        if (self->first_order){
            ...
            // Coordinate X
            const Primitive xleft_most  = prim_buff[tza * bx * by + tya * bx + (txa - 2)];
            const Primitive xleft_mid   = prim_buff[tza * bx * by + tya * bx + (txa - 1)];
            const Primitive center      = prim_buff[tza * bx * by + tya * bx + (txa + 0)];
            const Primitive xright_mid  = prim_buff[tza * bx * by + tya * bx + (txa + 1)];
            const Primitive xright_most = prim_buff[tza * bx * by + tya * bx + (txa + 2)];

            // Coordinate Y
            const Primitive yleft_most  = prim_buff[tza * bx * by + (tya - 2) * bx + txa];
            const Primitive yleft_mid   = prim_buff[tza * bx * by + (tya - 1) * bx + txa];
            const Primitive yright_mid  = prim_buff[tza * bx * by + (tya + 1) * bx + txa];
            const Primitive yright_most = prim_buff[tza * bx * by + (tya + 2) * bx + txa];

            // Coordinate z
            const Primitive zleft_most  = prim_buff[(tza - 2) * bx * by + tya * bx + txa];
            const Primitive zleft_mid   = prim_buff[(tza - 1) * bx * by + tya * bx + txa];
            const Primitive zright_mid  = prim_buff[(tza + 1) * bx * by + tya * bx + txa];
            const Primitive zright_most = prim_buff[(tza + 2) * bx * by + tya * bx + txa];
            ...
            case simbi::Geometry::SPHERICAL:
                {
                    ...
                    const real gb1  = prim_buff[txa + tya * bx + tza * bx * by].v1;
                    const real gb2  = prim_buff[txa + tya * bx + tza * bx * by].v2;
                    const real gb3  = prim_buff[txa + tya * bx + tza * bx * by].v3;
                    const real pc   = prim_buff[txa + tya * bx + tza * bx * by].p;
                    const real gam2 = (static_cast<real>(1.0) + (gb1 * gb2 + gb2 * gb2 + gb3 * gb3));
                    const real gam  = std::sqrt(gam2);
                    const real uc   = gb1 / gam;
                    const real vc   = gb2 / gam;
                    const real wc   = gb3 / gam;
