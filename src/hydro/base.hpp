...
namespace simbi
{
    struct HydroBase : public Managed<managed_memory>
    {
        ...
        void define_xactive_grid(bool is_first_order) {
            xactive_grid = (is_first_order) ? nx -2 : nx - 4;
        }

        void define_yactive_grid(bool is_first_order) {
            yactive_grid =  (is_first_order) ? ny -2 : ny - 4;
        }

        void define_zactive_grid(bool is_first_order) {
            zactive_grid = (is_first_order) ? nz -2 : nz - 4;
        }
        ...
        protected:
        // HydroBase() = default;                          // (3)
        // HydroBase(const HydroBase&) = delete;              // (1)
        // HydroBase& operator = (const HydroBase&) = delete; // (2)
        HydroBase(){}
        // ~HydroBase(){}
