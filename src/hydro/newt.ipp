    ...
    prims.transform(
        [gamma = this->gamma,
         loc   = &local_failure] DEV(auto& prim, const auto& cons_var)
            -> Maybe<primitive_t> {
            const real rho = cons_var.dens();
            const real v1  = cons_var.momentum(1) / rho;
            const real v2  = cons_var.momentum(2) / rho;
            const real v3  = cons_var.momentum(3) / rho;
            const real chi = cons_var.chi();
            const real pre =
                (gamma - 1.0) *
                (cons_var.nrg() - 0.5 * rho * (v1 * v1 + v2 * v2 + v3 * v3));
...
void Newtonian<dim>::adapt_dt()
{
    ...
    auto calc_local_dt = [geometry = this->geometry] DEV(
