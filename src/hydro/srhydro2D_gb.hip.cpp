                                 ...
                                 const luint nhat = 1) const
{
    ...
    const real vL = uL / std::sqrt(1 + uL * uL);
    const real vR = uR / std::sqrt(1 + uR * uR);
...
Conserved SRHD2D::prims2cons(const Primitive &prims) const
{
    ...
    const real lorentz_gamma = std::sqrt(static_cast<real>(1.0) + (u1 * u1 + u2 * u2));
    ...
    #pragma omp parallel 
    {
        ...
        for (luint jj = 0; jj < yphysical_grid; jj++)
        {
            ...
            for (luint ii = 0; ii < xphysical_grid; ii++)
            {
                ...
                const auto u1       = prims[aid].v1;
                const auto u2       = prims[aid].v2;
                const auto lorentz  = std::sqrt(1 + (u1 * u1 + u2 * u2));
                const auto v1       = u1 / lorentz;
                const auto v2       = u2 / lorentz;
...
{
    const real un              = prims.vcomponent(nhat);
    const auto kdelta          = kronecker(nhat, 1);
    const real rho             = prims.rho;
    const real u1              = prims.v1;
    const real u2              = prims.v2;
    const real lorentz_gamma   = std::sqrt(static_cast<real>(1.0) + (u1 * u1 + u2 * u2));
    const real vn              = un / lorentz_gamma;
    const real pressure        = prims.p;
    ...
    return Conserved{D * vn, S1 * vn + kdelta * pressure, S2 * vn + !kdelta * pressure, Sj - D * vn, D * vn * prims.chi};
};
    ...
    const real vface) const
{
    ...
    if (vface <= aStar)
    {
        ...
        const real uL         = left_prims.vcomponent(nhat);
        const real uO         = left_prims.vcomponent(!nhat);
        const real gL         = std::sqrt(1 + (uL * uL + uO * uO));
        const real vL         = uL / gL;
        const auto kdelta = kronecker(nhat, 1);
        // Left Star State in x-direction of coordinate lattice
        ...
        const real uR         = right_prims.vcomponent(nhat);
        const real uO         = right_prims.vcomponent(!nhat);
        const real gR         = std::sqrt(1 + (uR * uR + uO * uO));
        const real vR         = uR / gR;
        const auto kdelta     = kronecker(nhat, 1);
    ...
    simbi::parallel_for(p, (luint)0, nzones, [=] GPU_LAMBDA (luint gid){
        ...
        while (!found_failure && workLeftToDo)
        {
            ...
            const real v2     = S2 * inv_et;
                ...
                self->gpu_prims[gid]          = Primitive{rho, W * v1, W * v2, peq, Dchi / D};
            #else
                self->pressure_guess[gid] = peq;
                self->prims[gid]          = Primitive{rho, W * v1, W * v2, peq, Dchi / D};
            #endif
            // if (n == 0) {
    ...
    simbi::parallel_for(p, (luint)0, extent, [=] GPU_LAMBDA (const luint idx) {
        ...
        switch (geometry)
        {
            ...
            case simbi::Geometry::SPHERICAL:
                ...
                const real gb1  = prim_buff[txa * sy + tya * sx].v1;
                const real gb2  = prim_buff[txa * sy + tya * sx].v2;
                const real gam2 = (static_cast<real>(1.0) + (gb1 * gb1 + gb2 * gb2));
                const real uc   = gb1 / std::sqrt(gam2);
                const real vc   = gb2 / std::sqrt(gam2);
