...
struct InitialConditions {
    ...
    std::string data_directory, coord_system, solver, x1_cell_spacing, x2_cell_spacing, x3_cell_spacing;
    std::vector<std::string> boundary_conditions;
...
namespace sr1d {
    ...
    struct Conserved {
        ...
        GPU_CALLABLE_MEMBER Conserved operator +(const Conserved &cons) const {return Conserved(d + cons.d, s + cons.s, tau + cons.tau, chi + cons.chi); }
        GPU_CALLABLE_MEMBER Conserved operator -(const Conserved &cons) const {return Conserved(d - cons.d, s - cons.s, tau - cons.tau, chi - cons.chi); }
        GPU_CALLABLE_MEMBER Conserved operator /(const real c) const {return Conserved(d/c, s/c, tau/c, chi/c); }
        GPU_CALLABLE_MEMBER Conserved operator *(const real c) const {return Conserved(d*c, s*c, tau*c, chi*c); }
        GPU_CALLABLE_MEMBER Conserved & operator +=(const Conserved &cons) {
            d      += cons.d;
            s      += cons.s;
...
namespace rmhd3d {
    ...
    struct Primitive {
        ...
        GPU_CALLABLE_MEMBER Primitive() {}
        GPU_CALLABLE_MEMBER ~Primitive() {}
        GPU_CALLABLE_MEMBER Primitive(real rho, real v1, real v2, real v3, real p, real b1, real b2, real b3) : rho(rho), v1(v1), v2(v2), v3(v3), p(p), b1(b1), b2(b2), b3(b3), chi(0) {}
        GPU_CALLABLE_MEMBER Primitive(real rho, real v1, real v2, real v3, real p, real b1, real b2, real b3, real chi) : rho(rho), v1(v1), v2(v2), v3(v3), p(p), b1(b1), b2(b2), b3(b3), chi(chi) {}
        GPU_CALLABLE_MEMBER Primitive(const Primitive &c) : rho(c.rho), v1(c.v1), v2(c.v2), v3(c.v3), p(c.p), chi(c.chi) {}
        GPU_CALLABLE_MEMBER Primitive operator + (const Primitive &e)  const { return Primitive(rho+e.rho, v1+e.v1, v2+e.v2,v3+e.v3, p+e.p, b1 + e.b1, b2 + e.b2, b3 + e.b3, chi+e.chi); }  
        GPU_CALLABLE_MEMBER Primitive operator - (const Primitive &e)  const { return Primitive(rho-e.rho, v1-e.v1, v2-e.v2,v3-e.v3, p-e.p, b1 - e.b1, b2 - e.b2, b3 - e.b3, chi-e.chi); }  
        GPU_CALLABLE_MEMBER Primitive operator * (const real c)      const { return Primitive(rho*c, v1*c, v2*c,v3*c, p*c, b1*c, b2*c, b3*c, chi*c ); }
        GPU_CALLABLE_MEMBER Primitive operator / (const real c)      const { return Primitive(rho/c, v1/c, v2/c,v3/c, p/c, b1/c, b2/c, b3/c, chi/c ); }
