...
namespace simbi::mesh {
    ...
    struct mesh_config_t {
        ...
        vector_t<domain_t<Dims>, Dims> face_domain;   // face domains
        ...
        )
        {
            ...
            config.domain      = domain_algebra::contract(
                config.full_domain,
                ones<Dims, std::int64_t>() * config.halo_radius
            );

            for (std::uint64_t ii = 0; ii < Dims; ii++) {
                auto amount            = iarray<Dims>{0};
                amount[ii]             = 1;   // expand by 1 in this dimension
                config.face_domain[ii] = domain_algebra::expand_end(
                    make_domain(config.domain.shape()),
                    amount
                );
                if (init.is_mhd) {
