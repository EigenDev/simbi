...
namespace simbi::body::factory {
    ...
    struct binary_parameters_t {
            ...
            -> binary_parameters_t
        {
            ...
            return {
              ...
              .prescribed_motion = true   // default
            };
        }
    };
    ...
    template <std::uint64_t Dims>
    auto create_body_from_config(const config_dict_t& props)
        -> body_variant_t<Dims>
    {
    ...
    {
        auto collection = make_body_collection<Dims, MaxBodies>();

        for (const auto& body_config : init.immersed_bodies) {
            auto body  = create_body_from_config<Dims>(body_config);
    ...
    auto create_binary_system_from_config(const config_dict_t& sys_props)
    {
        ...
        auto body1 = create_body_from_config<Dims>(config1);
        auto body2 = create_body_from_config<Dims>(config2);

        return make_body_collection<Dims, MaxBodies>().add(body1).add(body2);
    }
    ...
    auto create_body_collection_from_init(const initial_conditions_t& init)
        -> std::optional<body_collection_t<Dims, MaxBodies>>
    {
        ...
        if (!init.immersed_bodies.empty() && !init.contains("body_system")) {
            return create_collection_from_individual_bodies<Dims, MaxBodies>(
                init
            );
        }
        ...
        if (init.contains("body_system")) {
            ...
            if (sys_props.contains("system_type")) {
                ...
                if (system_type == "binary") {
                    return create_binary_system_from_config<Dims, MaxBodies>(
                        sys_props
                    );
                }
                else {
    ...
    // convenience aliases
    template <std::uint64_t Dims>
    using default_body_collection = body_collection_t<Dims, 8>;

    template <std::uint64_t Dims>
    auto create_default_collection_from_init(const initial_conditions_t& init)
    {
        return create_body_collection_from_init<Dims, 8>(init);
    }
