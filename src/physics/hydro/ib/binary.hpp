...
namespace simbi::ibsystem::body_functions {
    namespace binary {
        ...
        std::tuple<vector_t<T, Dims>, vector_t<T, Dims>>
            ...
            requires(Dims >= 2)
        {
            ...
            if constexpr (Dims == 2) {
                vector_t<T, Dims> r1{a1, 0.0};
                vector_t<T, Dims> r2{-a2, 0.0};
                return {r1, r2};
            }
            else {
                vector_t<T, Dims> r1{a1, 0.0, 0.0};
                vector_t<T, Dims> r2{-a2, 0.0, 0.0};
                return {r1, r2};
            }
        }

        template <std::uint64_t Dims, typename T>
        std::tuple<vector_t<T, Dims>, vector_t<T, Dims>> initial_velocities(
            T semi_major,
            T total_mass,
            T mass_ratio,
            bool circular_orbit
            ...
            requires(Dims >= 2)
        {
            if (circular_orbit) {
                ...
                if constexpr (Dims == 2) {
                    vector_t<T, Dims> v1{0.0, phi_dot * a2};
                    vector_t<T, Dims> v2{0.0, -phi_dot * a1};
                    return {v1, v2};
                }
                else {
                    vector_t<T, Dims> v1{0.0, phi_dot * a2, 0.0};
                    vector_t<T, Dims> v2{0.0, -phi_dot * a1, 0.0};
                    return {v1, v2};
                }
            }
            else {
                ...
                if constexpr (platform::is_gpu) {
                    ...
                    return {vector_t<T, Dims>(), vector_t<T, Dims>()};
                }
                else {
        ...
        vector_t<Body<T, Dims>, 2> calculate_binary_motion(
