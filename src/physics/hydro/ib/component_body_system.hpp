...
namespace simbi::ibsystem {
    ...
    class ComponentBodySystem : public managed_t<global::managed_memory>
    {
      public:
        ...
        ComponentBodySystem<T, Dims> add_body(const Body<T, Dims>& body) const
        {
            ...
            if (body.gravitational.has_value()) {
                new_system.grav_body_indices_ =
                    new_system.grav_body_indices_.concat(
                        new_system.bodies_.size() - 1
                    );
            }

            if (body.accretion.has_value()) {
                new_system.accr_body_indices_ =
                    new_system.accr_body_indices_.concat(
                        new_system.bodies_.size() - 1
                    );
            }
        ...
        DUAL const field_t<Body<T, Dims>>& bodies() const { return bodies_; }
        ...
        bool invokes_gravity() const { return !grav_body_indices_.null(); }
        bool invokes_accretion() const { return !accr_body_indices_.null(); }
        ...
        field_t<std::variant<
            ...
            PropertyDescriptor<vector_t<T, Dims>>>>
        get_serializable_properties(size_t body_idx) const
        {
            ...
            field_t<PropertyVariant> properties;
            ...
            properties.push_back(
                PropertyDescriptor<vector_t<T, Dims>>{"force", [body](size_t) {
                                                          return body.force;
                                                      }}
            );
        ...
        {
            bodies_.to_gpu();
            grav_body_indices_.to_gpu();
            accr_body_indices_.to_gpu();
        }
      ...
      private:
        ...
        field_t<Body<T, Dims>> bodies_;

        // GPU-optimized lookup arrays
        field_t<size_t> grav_body_indices_;
        field_t<size_t> accr_body_indices_;
