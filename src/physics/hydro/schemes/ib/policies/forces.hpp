 ...
 *
 * @file            forces.hpp
 * @brief           forces and interactions for the IB scheme
 * @details
 *
 * @version         0.8.0
 * @date            2025-05-11
...
namespace simbi::ib::forces {
    struct GravitationalForcePolicy {
        template <typename T, size_type Dims, typename Primitive>
        DEV Primitive::counterpart_t operator()(
        ...
        ) const
        {
            ...
            const T mass         = system.masses()[body_idx];
            const T softening    = system.softening_length()[body_idx];

            // Reset force accumulator
            system.force_ref()[body_idx] = spatial_vector_t<T, Dims>();

            // Apply gravitational force to entire fluid domain
            const auto r  = mesh_cell.cartesian_centroid() - position;
            ...
            const auto f_cart = mass * r / (r2 * std::sqrt(r2));
            ...
            if (system.two_way_coupling(body_idx)) {
                // TODO: since I am learning about functional
                // style programming we'd pass this info to another
                // function to handle the force application
                // something like: BodyForcesTracker::record_reaction_force(...)
            }
            return state;
        }
    };

    // accretion effects policy
    struct AccretionInteractionPolicy {
        ...
        template <typename T, size_type Dims, typename Primitive>
        Primitive::counterpart_t operator()()
        {
        }
    };
