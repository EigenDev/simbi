...
namespace simbi::body_functions {
    namespace gravitational {
        ...
        DEV Primitive::counterpart_t apply_gravitational_force(
            const spatial_vector_t<T, Dims>& body_position,
            T body_mass,
            T softening_length,
            bool two_way_coupling,
            // spatial_vector_t<T, Dims>& body_force_ref,
    ...
    namespace accretion {
        ...
        template <typename T, size_type Dims, typename Primitive>
        DEV Primitive::counterpart_t apply_accretion_effect(
            const spatial_vector_t<T, Dims>& body_position,
            const spatial_vector_t<T, Dims>& body_velocity,
            T body_mass,
            // T& body_mass_ref,   // Reference to update body mass
            T accretion_efficiency,
            T accretion_radius,
            // T& total_accreted_mass_ref,
            const Primitive& prim,
            const Cell<Dims>& mesh_cell,
            const HydroContext& context,
            T dt
        )
        {
            ...
            const auto r_vector =
                mesh_cell.cartesian_centroid() - body_position;
            ...
            const auto r_bondi    = 2.0 * body_mass / (cs_ambient * cs_ambient);
            ...
            const auto rel_velocity = prim.velocity() - body_velocity;
            ...
            if (accretion_factor > 0) {
                ...
                // const T accreted_mass = accreted_density * dV;

                // Update tracking variables (do this the functional way)
                // total_accreted_mass_ref += accreted_mass;
                // body_mass_ref += accreted_mass;
    ...
    namespace binary {
        ...
        template <typename T, size_type Dims>
        void update_binary_prescribed_motion(
        ...
        )
        {
            ...
            // Calculate masses
            const T m1         = system.masses()[body1_idx];
            const T m2         = system.masses()[body2_idx];
            ...
            T phi     = phi_dot * current_time;

            // Update positions and velocities
            system.positions_mut()[body1_idx]  = vecops::rotate(pos1, phi);
