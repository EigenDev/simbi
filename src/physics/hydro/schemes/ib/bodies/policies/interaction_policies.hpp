...
namespace simbi::ib {
    ...
    {
      private:
        using trait_t = traits::Accreting<T>;

      public:
        using Params = typename trait_t::Params;

        DUAL AccretingFluidInteractionPolicy(const Params& params = {})
            : trait_(params), drag_coefficient_(0.47)
        {
        }

        // Access to the accreting trait
        DUAL const trait_t& accreting_trait() const { return trait_; }
        DUAL trait_t& accreting_trait() { return trait_; }

        // Forward trait methods
        DUAL T accretion_efficiency() const
        {
            return trait_.accretion_efficiency();
        }
        DUAL T accretion_radius_factor() const
        {
            return trait_.accretion_radius_factor();
        }
        DUAL T total_accreted_mass() const
        {
            return trait_.total_accreted_mass();
        }
        ...
        calculate_fluid_forces(Body& body, const auto& mesh, const T dt)
        {
            ...
            if (v_norm > 0) {
                ...
                // Full drag equation: F = -0.5 * rho * Cd * A * v^2 * (v/|v|)
                const auto drag_force = -0.5 * avg_density * drag_coefficient_ *
                                        area * v_norm * v_rel;
        ...
        )
        {
            const auto accretion_radius =
                body.radius() * trait_.accretion_radius_factor();
            ...
            for (const auto& idx : body.cut_cell_indices()) {
                ...
                if (std::abs(cell.distance) <= accretion_radius) {
                    ...
                    const T max_accretion_fraction =
                        trait_.accretion_efficiency() * cell.volume_fraction;
            ...
            if (total_accreted_mass > 0) {
                ...
                trait_.add_accreted_mass(total_accreted_mass);
                trait_.add_accreted_momentum(total_accreted_momentum.norm());
                trait_.add_accreted_energy(total_accreted_energy);
            }
        }
        ...
        {
            // Perform accretion if needed
            if (trait_.accretion_efficiency() > 0) {
                accrete(body, cons_states, prim_states);
            }
        }

        DUAL const Params& params() const { return trait_.params(); }
        DUAL Params& params() { return trait_.params(); }
            ...
            cons_states.transform_with_indices(
                [=,
                 body_ptr =
                    ...
                    const auto mesh_cell = mesh.get_cell_from_global(idx);
                    const auto r  = mesh_cell.centroid() - body_ptr->position_;
