...
// ghost.hpp
//
// utilities for analyzing and classifying ghost cell regions.
// defines types like `ghost_region_t` and `ghost_set_t` to describe the
// regions of a grid domain that are ghost cells. `analyze_ghost_regions` is
// the main function that identifies and classifies these regions as face, edge,
// or corner-adjacent.
//
// usage:
//   auto ghost_regions = analyze_ghost_regions(full_domain, active_domain);
...
namespace simbi::grid::boundary {
    ...
    template <std::uint64_t Rank>
    constexpr ghost_type_t
    classify_ghost_type(const domain_t<Rank>& ghost, const domain_t<Rank>& active)
    {
        std::uint64_t contact_count = 0;
        for (std::uint64_t dim = 0; dim < Rank; ++dim) {
            if (ghost.start[dim] == active.fin[dim] || ghost.fin[dim] == active.start[dim]) {
                contact_count++;
            }
        }
    ...
    template <std::uint64_t Rank>
    constexpr auto ghost_direction(const domain_t<Rank>& ghost, const domain_t<Rank>& active)
    {
        vector_t<face_side_t, Rank> directions;
        for (std::uint64_t dim = 0; dim < Rank; ++dim) {
    ...
    template <std::uint64_t Rank>
    auto
    analyze_ghost_regions(const domain_t<Rank>& full_domain, const domain_t<Rank>& active_domain)
    {
