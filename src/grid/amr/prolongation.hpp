...
namespace simbi::grid::amr {
    ...
    DUAL T limit_slope(const T& slope_left, const T& slope_right)
    {
        ...
        if constexpr (is_hydro_primitive_c<T> || is_hydro_conserved_c<T>) {
            ...
            const auto     r     = (std::abs(sl) < build::epsilon) ? 1.0 : sr / sl;
            constexpr real theta = 2.0;
            return sl * my_max(0.0, my_min(theta * r, my_min((1.0 + r) * 0.5, theta)));
        }
    }
    ...
    struct prolong_constant_t
    {
        ...
        DUAL prolong_constant_t(CoarseComp comp, argument_type ratio, domain_t<Rank> coarse_domain)
            : coarse_comp_(std::move(comp)), ratio_(ratio), coarse_domain_(coarse_domain)
        {
        }
    ...
    struct prolong_linear_t
    {
        ...
        DUAL prolong_linear_t(CoarseComp comp, argument_type ratio, domain_t<Rank> coarse_domain)
            : coarse_comp_(std::move(comp)), ratio_(ratio), coarse_domain_(coarse_domain)
        {
            for (std::uint64_t d = 0; d < Rank; ++d) {
        ...
        DUAL value_type operator()(const argument_type& fine_coord) const
        {
            ...
            for (std::uint64_t d = 0; d < Rank; ++d) {
                ...
                std::int64_t local         = fine_coord[d] - coarse_coord[d] * ratio_[d];
                real         fine_center   = static_cast<real>(local) + 0.5;
                real         coarse_center = static_cast<real>(ratio_[d]) * 0.5;
                normalized_offset[d]       = (fine_center - coarse_center) * inv_ratio_[d];
            }
            ...
            for (std::uint64_t d = 0; d < Rank; ++d) {
                if (coarse_coord[d] < coarse_domain_.start[d] ||
                    ...
                    coarse_coord[d] = simbi::helpers::my_max(
                        ...
                        simbi::helpers::my_min(coarse_coord[d], coarse_domain_.fin[d] - 1)
                    );
                }
            }
            ...
            for (std::uint64_t dd = 0; dd < Rank; ++dd) {
                ...
                if constexpr (is_hydro_primitive_c<value_type> ||
                    ...
                    result = result | structs::add_gas(slope * normalized_offset[dd]);
                }
                else {
    ...
    struct prolong_parabolic_t
    {
        ...
        DUAL prolong_parabolic_t(CoarseComp comp, argument_type ratio, domain_t<Rank> coarse_domain)
            : coarse_comp_(std::move(comp)), ratio_(ratio), coarse_domain_(coarse_domain)
        {
            for (std::uint64_t d = 0; d < Rank; ++d) {
                inv_ratio_[d] = 1.0 / static_cast<real>(ratio[d]);
        ...
        DUAL value_type operator()(const argument_type& fine_coord) const
        {
            ...
            for (std::uint64_t d = 0; d < Rank; ++d) {
                ...
                std::int64_t offset = fine_coord[d] - coarse_coord[d] * ratio_[d];
                x_norm[d]           = (static_cast<real>(offset) + 0.5) * inv_ratio_[d] - 0.5;
            }
            ...
            for (std::uint64_t dd = 0; dd < Rank; ++dd) {
                ...
                if (has_left && has_right) {
                    ...
                    if constexpr (is_hydro_primitive_c<value_type> ||
                        ...
                        result = result + (d2v_limited * 0.5) * parabolic_factor;
                    }
                }
                else {
    ...
    auto prolong(const Computation& coarse, iarray<Rank> ratio)
    {
        ...
        if constexpr (Order == 1) {
            auto kernel = prolong_constant_t<Computation, Rank>(coarse, ratio, coarse_domain);
            return computation_t<Rank, decltype(kernel)>{std::move(kernel), fine_domain};
        }
        else if constexpr (Order == 2) {
            auto kernel = prolong_linear_t<Computation, Rank>(coarse, ratio, coarse_domain);
            return computation_t<Rank, decltype(kernel)>{std::move(kernel), fine_domain};
        }
        else if constexpr (Order == 3) {
            auto kernel = prolong_parabolic_t<Computation, Rank>(coarse, ratio, coarse_domain);
            return computation_t<Rank, decltype(kernel)>{std::move(kernel), fine_domain};
        }
        else {
