...
double UstateSR::adapt_dt(vector<vector<double> > &u_state, vector<double> &r, 
    ...
    for (int ii = 0; ii < physical_grid; ii++){
        ...
        cs = calc_rel_sound_speed(p, D, tau, W[shift_i], gamma);

        cfl_dt = dr/(max(abs(v + cs), abs(v - cs)));
                                            ...
                                            double lorentz_right)
{
    ...
    prims_l = cons2prim(gamma, left_state, lorentz_left);
    prims_r = cons2prim(gamma, right_state, lorentz_right);
    ...
    cs_r = calc_rel_sound_speed(p_r, right_state[0], right_state[2], lorentz_right, gamma);
    cs_l = calc_rel_sound_speed(p_l, left_state[0], left_state[2], lorentz_left, gamma);
                                        ...
                                        vector<double> right_flux)
{
    ...
    lambda = calc_eigenvals(gamma, left_state, right_state, lorentz_left, lorentz_right);
...
vector<vector<double> > UstateSR::u_dot1D(vector<vector<double> > &u_state, vector<double> &lorentz_gamma,
    ...
    if (first_order){
        ...
        if (coord_system == default_coordinates) {
            ...
            for (int ii = i_start; ii < i_bound; ii++){
                ...
                f1 = calc_hll_flux(gamma, W_l, W_r, u_l, u_r, f_l, f_r);
                ...
                // Calc HLL Flux at i-1/2 interface
                f2 = calc_hll_flux(gamma, W_l, W_r, u_l, u_r, f_l, f_r);
 ...
 vector<vector<double> > UstateSR::simulate1D(vector<double> &lorentz_gamma, float tend = 0.1, float dt = 1.e-4, float theta=1.5,
    ...
    if (first_order){
        ...
        while (t < tend){
            ...
            udot = u_dot1D(u, lorentz_gamma, true, periodic, theta, linspace);
            ...
            // Adjust the timestep 
            if (t > 0){
                dt = adapt_dt(u_p, r, lorentz_gamma, linspace, first_order);
            }
        ...
        while (t < tend){
            ...
            udot = u_dot1D(u, lorentz_gamma, false, periodic, theta, linspace);
            ...
            udot1 = u_dot1D(u1, lorentz_gamma, false, periodic, theta, linspace);
            ...
            udot2 = u_dot1D(u2, lorentz_gamma, false, periodic, theta, linspace);
