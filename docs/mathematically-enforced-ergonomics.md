...
| EOS scalar arguments | positional bare `S`: `sound_speed(rho, pre)`, `pressure(rho, e_int)` — two different second-argument meanings on one trait | swapped or confused arguments compile | done: the `Eos` doors take `Density` / `Pressure` / `SpecificInternalEnergy` / `VelocitySquared`, and the stored conserved slot is the associated `Eos::ConservedQuantity` — `EnergyDensity` for the gamma-law closures, `SoundSpeedSquared` for isothermal — crossed only through the named `StoredQuantity` doors at the state-storage boundary. arguments carry identity; returns stay bare scalars except the stored quantity, whose meaning varies per closure and therefore lives in the associated type. swapped, e_int/nrg-confused, and cs^2-as-energy calls are compile-fail-pinned |
