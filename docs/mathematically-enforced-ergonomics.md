...
| face normal in `to_flux` / wave speeds | `nhat: &Tensor<S, D>`, unit length and frame/variance role by convention | a non-unit or wrong-frame vector projects fluxes | `Normalized<R::Normal>` at the regime interface, with the regime selecting the lawful physical or coordinate-frame witness |
