...
fn rmhd_wave_speed_map_bounds_cpp_quartic() {
    ...
    // the map now traces `rmhd_magnetosonic_cfl_speeds` (the cheap c_f^2 upper bound), NOT the
    // exact Mignone & Del Zanna quartic — the quartic stays on the Riemann/flux path. so the
    // contract is no longer EQUALITY with the C++ quartic but CFL-SAFETY: the bound must never
