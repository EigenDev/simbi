...
//
// **B6-iv Phase 4b proof**: configuring the substrate kernel-set with a
...
// moves in the prescribed direction by ~ ρ·g·t over the run). bit-equivalence
// vs the unfused path is covered by the lower-layer Phase 2/2b/3/4 tests; this
...
fn adiabatic_evolve_with_fused_uniform_accel_actually_accelerates_gas() {
    // **end-to-end Phase 4b claim**: a `with_fused_source` binding causes the
    ...
    // initially-stationary gas develops a +x velocity proportional to g_ext·t.
    //
    ...
    //     mom(t) = mom(0) + ρ·g_ext·t      = ρ·g_ext·t  (starting from rest)
    // which integrates to a velocity v(t) ≈ g_ext·t. for g_ext = 0.5 and
    // t_final ≈ 0.05, v ≈ 0.025 — clearly above any numerical noise floor
    // and FAR less than cs ≈ 1, so the flow stays subsonic + smooth.
    let n = 32usize;
    let dx = 1.0 / n as f64;
    ...
    // **Phase 4b binding**: a uniform_accel SourceSpec mapping `g_ext_0 = 0.5`.
    ...
    // step inside evolve() — there is no manual `cons.mom += dt·ρ·g_ext` loop
    // outside the substrate. if Phase 4b wiring works, gas accelerates;
    ...
    // average is close to the analytical g_ext·t. the boundary cells drift
