...
def run(cmd, **kwargs):
    ...
    try:
        ...
        duration = time.time() - start_time
        print(
            f"error: command timeout after {duration:.1f}s: {' '.join(str(c) for c in cmd)}",
            file=sys.stderr,
        )
        ...
        duration = time.time() - start_time
        print(
            f"error: command failed after {duration:.1f}s: {' '.join(str(c) for c in cmd)}",
            file=sys.stderr,
        )
        ...
        if "nvcc" in str(cmd) or "hipcc" in str(cmd):
            ...
            print(
                "  - verifying device architecture: --device-arch",
                file=sys.stderr,
            )
        elif "meson" in str(cmd):
            print("hint: build configuration failed. try:", file=sys.stderr)
            print(
                "  - clean rebuild: ./dev.py clean && ./dev.py build",
                file=sys.stderr,
            )
            print(
                "  - check dependencies with: meson setup --wipe",
                file=sys.stderr,
            )
...
def build_command(args):
    ...
    if args.verbose:
        ...
        print(
            f"  fast linker: {capabilities['fast_linker'] or 'system default'}"
        )
        print()
    ...
    needs_reconfigure = (
        ...
        or (
            args.gpu
            and not (build_dir / "meson-info/intro-dependencies.json").exists()
        )
    )
    ...
    if needs_reconfigure:
        ...
        if args.gpu:
            if not capabilities["gpu_backend"]:
                print(
                    "error: gpu compilation requested but no gpu backend available",
                    file=sys.stderr,
                )
                print(
                    "install cuda toolkit (nvcc) or rocm (hipcc)",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            print(
                f"enabling gpu compilation with {capabilities['gpu_backend']} backend"
            )
        ...
        # state field layout
        if args.layout:
            setup_cmd.append(f"-Dstate_layout={args.layout}")
            if args.layout == "soa":
                print("using SoA memory layout (optimized for GPU coalescing)")
    ...
    if not args.configure_only:
        ...
        if args.gpu:
                ...
                compile_env.update(
                    {
                        ...
                        "CUDA_CACHE_MAXSIZE": str(
                            min(1024, capabilities["memory_gb"] * 128)
                        ),
                    }
                )
            ...
            cpu_jobs = min(
                capabilities["cpu_count"], capabilities["memory_gb"] * 2
            )
...
def install_command(args):
    ...
    if not args.configure_only:
        ...
        run(
            ["meson", "install", "-C", args.build_dir],
            env=install_env,
        ...
        try:
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "import simbi; print('installation verified')",
                ],
                capture_output=True,
                text=True,
                timeout=10,
            ...
            if result.returncode == 0:
                ...
                print(
                    "warning: installation validation failed", file=sys.stderr
                )
        except subprocess.TimeoutExpired:
...
def main():
    ...
    build_parser.add_argument(
        "--linker",
        choices=["auto", "mold", "lld", "gold", "bfd"],
        default="auto",
    ...
    build_parser.add_argument(
        "--four-velocity",
        action="store_true",
        help="use four-velocity primitive",
    )
    build_parser.add_argument(
        "--unified-memory",
        action="store_true",
        help="use CUDA unified memory (default: device memory)",
    )
    build_parser.add_argument(
        "--layout",
        choices=["aos", "soa"],
        default="aos",
        help="memory layout for state fields (aos=array-of-structs, soa=struct-of-arrays)",
    )
    build_parser.add_argument(
    ...
    build_parser.add_argument(
        "--configure-only",
        action="store_true",
        help="configure without compiling",
    )
    build_parser.add_argument(
        "--gpu-jobs",
        type=int,
        help="max parallel jobs for gpu compilation (auto-detected if not specified)",
    )
    build_parser.add_argument(
        "--timeout",
        type=int,
    ...
    clean_parser = subparsers.add_parser(
        "clean", help="clean build directory and artifacts"
    )
    clean_parser.add_argument(
        "--all",
        action="store_true",
        help="clean all generated files including extensions and cache",
    )
